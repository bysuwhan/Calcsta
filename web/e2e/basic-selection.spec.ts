...
async function openBasicUi(page: Page) {
...
test.describe('the Selection ribbon command', () => {
  test.beforeEach(async ({ page }) => {
    await openBasicUi(page);
    await loadModel(page, 'two-story-frame');
  });

  for (const tool of ['node', 'element', 'support', 'load'] as const) {
    test(`disarms the ${tool} edit tool`, async ({ page }) => {
      await page.getByTestId(`rb-cmd-${tool}`).click();
      await expect.poll(() => page.evaluate(() => window.__stabileo.currentTool()))
        .toBe(tool);
  ...
  test('keeps the chosen selection kinds when returning from an edit tool', async ({ page }) => {
    await page.getByTestId('rb-cmd-select').click();
    await page.getByTestId('select-mode-nodes').click();
    await expect.poll(() => page.evaluate(() => window.__stabileo.armedKinds()))
      .toEqual(['nodes']);
    ...
    expect(await page.evaluate(() => window.__stabileo.armedKinds())).toEqual(['nodes']);
  });

  test('does not mutate the model on the next canvas click', async ({ page }) => {
    await page.getByTestId('rb-cmd-element').click();
    await page.getByTestId('rb-cmd-select').click();
  ...
  test('cancels an unfinished element chain', async ({ page }) => {
    const before = await page.evaluate(() => window.__stabileo.elementIds().length);
    await page.getByTestId('rb-cmd-element').click();
    const first = await page.evaluate(() => window.__stabileo.nodeScreenPos(1));
    if (!first) throw new Error('node 1 is not visible');
    await page.mouse.click(first.x, first.y);

    await page.getByTestId('rb-cmd-select').click();
    await page.getByTestId('rb-cmd-element').click();
    const second = await page.evaluate(() => window.__stabileo.nodeScreenPos(6));
    if (!second) throw new Error('node 6 is not visible');
    await page.mouse.click(second.x, second.y);

    expect(await page.evaluate(() => window.__stabileo.elementIds().length)).toBe(before);
  });
});
...
test.describe('box select in Basic 2D', () => {
  ...
  test('no pressed filters means a drag takes every kind', async ({ page }) => {
    await expect(page.getByTestId('selection-filter-bar')).toBeVisible();
    for (const kind of ['nodes', 'elements', 'supports', 'loads']) {
      await expect(page.getByTestId(`select-mode-${kind}`)).toHaveAttribute('aria-pressed', 'false');
    }
    await sweepAll(page);
    const got = await picked(page);
    for (const kind of ['nodes', 'elements', 'supports', 'loads'] as const) {
      expect(got[kind].length, kind).toBeGreaterThan(0);
    }
  });

  test('selection filters stay visible and become disabled outside the select tool', async ({ page }) => {
    await page.getByTestId('rb-cmd-node').click();
    await expect(page.getByTestId('selection-filter-bar')).toBeVisible();
    await expect(page.getByTestId('select-mode-nodes')).toBeDisabled();

    await page.getByTestId('rb-cmd-select').click();
    await expect(page.getByTestId('select-mode-nodes')).toBeEnabled();
  });
  ...
  test('pressing two filters makes a drag take both', async ({ page }) => {
  ...
  test('turning the last filter off returns to the all-kinds default', async ({ page }) => {
    const members = page.getByTestId('select-mode-elements');
    await members.click();
