<script lang="ts">
  ...
  const fmtUnit = (value: number, quantity: Quantity) => fmtForce(toDisplay(value, quantity, us));
  const setCustomForce = (field: 'n' | 'vy' | 'vz', raw: string) => {
    const value = parseFloat(raw);
    if (Number.isFinite(value)) eccCustom = { ...eccCustom, [field]: fromDisplay(value, 'force', us) };
  };
    ...
    <div class="ssp-body">
      ...
      {#if is3D && analysis3D}
        ...
        <div class="ssp-forces">
          <div class="ssp-force">
            ...
            <span class="ssp-force-value">{fmtUnit(analysis3D.N, 'force')} {unitLabel('force', us)}</span>
          </div>
          <div class="ssp-force">
            <span class="ssp-force-label">V<sub>y</sub></span>
            <span class="ssp-force-value">{fmtUnit(analysis3D.Vy, 'force')} {unitLabel('force', us)}</span>
          </div>
          <div class="ssp-force">
            <span class="ssp-force-label">V<sub>z</sub></span>
            <span class="ssp-force-value">{fmtUnit(analysis3D.Vz, 'force')} {unitLabel('force', us)}</span>
          </div>
        ...
        <div class="ssp-forces ssp-forces-moments">
          <div class="ssp-force">
            ...
            <span class="ssp-force-value">{fmtUnit(-analysis3D.Mx, 'moment')} {unitLabel('moment', us)}</span>
          </div>
          <div class="ssp-force">
            <span class="ssp-force-label">M<sub>y</sub></span>
            <span class="ssp-force-value">{fmtUnit(-analysis3D.My, 'moment')} {unitLabel('moment', us)}</span>
          </div>
          <div class="ssp-force">
            <span class="ssp-force-label">M<sub>z</sub></span>
            <span class="ssp-force-value">{fmtUnit(-analysis3D.Mz, 'moment')} {unitLabel('moment', us)}</span>
          </div>
      ...
      {:else if analysis2D}
        <div class="ssp-forces">
          <div class="ssp-force">
            <span class="ssp-force-label">N</span>
            <span class="ssp-force-value">{fmtUnit(analysis2D.N, 'force')} {unitLabel('force', us)}</span>
          </div>
          <div class="ssp-force">
            <span class="ssp-force-label">V</span>
            <span class="ssp-force-value">{fmtUnit(analysis2D.V, 'force')} {unitLabel('force', us)}</span>
          </div>
          <div class="ssp-force">
            <span class="ssp-force-label">M</span>
            <span class="ssp-force-value">{fmtUnit(-analysis2D.M, 'moment')} {unitLabel('moment', us)}</span>
          </div>
        ...
        <div class="ssp-ecc">
          ...
          <div class="ssp-ecc-fields">
            <div class="ssp-ecc-field">
              ...
              {#if eccSource === 'custom'}
                <input type="number" step="any" value={toDisplay(eccCustom.n, 'force', us)} oninput={(e) => setCustomForce('n', e.currentTarget.value)} />
              {:else}
                <span class="ssp-ecc-fixed">{fmtUnit(eccentricComponents.n, 'force')}</span>
              {/if}
              <span class="ssp-ecc-unit">{unitLabel('force', us)}</span>
            </div>
            <div class="ssp-ecc-field">
              <span class="ssp-ecc-flabel">V<sub>y</sub> <em>{t('stress.eccentricParH')}</em></span>
              {#if eccSource === 'custom'}
                <input type="number" step="any" value={toDisplay(eccCustom.vy, 'force', us)} oninput={(e) => setCustomForce('vy', e.currentTarget.value)} />
              {:else}
                <span class="ssp-ecc-fixed">{fmtUnit(eccentricComponents.vy, 'force')}</span>
              {/if}
              <span class="ssp-ecc-unit">{unitLabel('force', us)}</span>
            </div>
            <div class="ssp-ecc-field">
              ...
              {#if eccSource === 'custom'}
                <input type="number" step="any" value={toDisplay(eccCustom.vz, 'force', us)} oninput={(e) => setCustomForce('vz', e.currentTarget.value)} />
              {:else}
                <span class="ssp-ecc-fixed">{fmtUnit(eccentricComponents.vz, 'force')}</span>
              {/if}
              <span class="ssp-ecc-unit">{unitLabel('force', us)}</span>
            </div>
          </div>
          ...
          <div class="ssp-ecc-row">
            ...
            <span class="ssp-ecc-val">
              y {fmtUnit(eccentricPoint[0], 'length')} · z {fmtUnit(eccentricPoint[1], 'length')} {unitLabel('length', us)}
            </span>
          </div>
          {#if eccentricHasParallel && eccentricPointV}
            <div class="ssp-ecc-row">
              ...
              <span class="ssp-ecc-val">
                y {fmtUnit(eccentricPointV[0], 'length')} · z {fmtUnit(eccentricPointV[1], 'length')} {unitLabel('length', us)}
              </span>
            </div>
          {/if}
          ...
          <div class="ssp-ecc-row">
            ...
            <span class="ssp-ecc-val">
              {fmtUnit(eccentric.effect.myFromN, 'moment')} / {fmtUnit(eccentric.effect.mzFromN, 'moment')} {unitLabel('moment', us)}
            </span>
          </div>
          <div class="ssp-ecc-row" class:ssp-ecc-warn={Math.abs(eccentric.effect.tFromShear) > 1e-6}>
            <span class="ssp-ecc-label">&Delta;T</span>
            <span class="ssp-ecc-val">{fmtUnit(eccentric.effect.tFromShear, 'moment')} {unitLabel('moment', us)}</span>
          </div>
        ...
        <div class="ssp-shear-warn" role="alert">
          ...
          <div>
            ...
            <p class="ssp-shear-warn-nums">
              {t('stress.shearDiagram')}: {fmtUnit(shearCheck.closedForm, 'stress')} {unitLabel('stress', us)} ·
              {t('stress.shearSolved')}: {fmtUnit(shearCheck.solved, 'stress')} {unitLabel('stress', us)}
            </p>
          </div>
        </div>
      {/if}
...
<style>
  ...
  */
  ...
  .ssp-ecc-reset:hover { background: rgba(255, 140, 0, 0.5); }
  .ssp-ecc-row {
    display: flex;
