<script lang="ts">
  ...
  const unitSystem = $derived(uiStore.unitSystem);
  ...
  $effect(() => {
    if (sec) {
      localName = sec.name;
      localA = String(toDisplay(sec.a, 'area', unitSystem));
      localIz = String(toDisplay(sec.iz, 'inertia', unitSystem));
      localB = sec.b != null ? String(toDisplay(sec.b, 'length', unitSystem)) : '';
      localH = sec.h != null ? String(toDisplay(sec.h, 'length', unitSystem)) : '';
  ...
  let autoCalc = $derived(() => {
    ...
    if (!isNaN(b) && !isNaN(h) && b > 0 && h > 0) {
      const bSI = fromDisplay(b, 'length', unitSystem);
      const hSI = fromDisplay(h, 'length', unitSystem);
      return {
        a: toDisplay(bSI * hSI, 'area', unitSystem),
        iz: toDisplay((bSI * hSI * hSI * hSI) / 12, 'inertia', unitSystem),
      };
    }
    return null;
  });
  ...
  function confirm() {
    ...
    const updates: Record<string, any> = {
      name: localName,
      a: fromDisplay(a, 'area', unitSystem),
      iz: fromDisplay(iz, 'inertia', unitSystem),
    };
    const b = parseFloat(localB);
    const h = parseFloat(localH);
    if (!isNaN(b)) updates.b = fromDisplay(b, 'length', unitSystem);
    if (!isNaN(h)) updates.h = fromDisplay(h, 'length', unitSystem);
  ...
  function handleProfileSelect(profile: SteelProfile, _section: { a: number; iz: number; b: number; h: number }) {
    ...
    localA = String(toDisplay(full.a, 'area', unitSystem));
    localIz = String(toDisplay(full.iz, 'inertia', unitSystem));
    localB = String(toDisplay(full.b, 'length', unitSystem));
    localH = String(toDisplay(full.h, 'length', unitSystem));
...
{#if sec}
  ...
  <div class="editor">
    ...
    <div class="field">
      <span>A ({unitLabel('area', unitSystem)}):</span>
      <input
        type="number"
        step="any"
        bind:value={localA}
    ...
    <div class="field">
      <span>{t('secEdit.iz')} ({unitLabel('inertia', unitSystem)}):</span>
      <input
        type="number"
        step="any"
        bind:value={localIz}
    ...
    <div class="field">
      <span>b ({unitLabel('length', unitSystem)}):</span>
      <input
        type="number"
        step="0.001"
        bind:value={localB}
    ...
    <div class="field">
      <span>h ({unitLabel('length', unitSystem)}):</span>
      <input
        type="number"
        step="0.001"
        bind:value={localH}
