<script lang="ts">
  ...
  import ToolDimensionOptions from '../floating-tools/ToolDimensionOptions.svelte';
   ...
   */
  const HAS_OPTIONS = ['select', 'node', 'element', 'fixed', 'dimension', 'support', 'load', 'cylinder', 'influenceLine'];
...
<div class="tool-bar" data-testid="tool-options-bar">
  <div class="tb-opts" data-testid="tool-options">
    {#if blockUI.hint}
      <span class="tb-tool-name">
        {t(blockUI.command?.kind === 'move'
          ? 'ribbon.blockMove'
          : blockUI.command?.kind === 'rotate'
            ? 'ribbon.blockRotate'
            : 'ribbon.blockCreate')}
      </span>
      <span class="tb-sep" aria-hidden="true"></span>
      <span class="tb-command-hint" role="status">{blockUI.hint}</span>
    {:else if showOptions}
      <span class="tb-tool-name">{uiStore.currentTool === 'cylinder' ? '실린더' : t(`float.${uiStore.currentTool}`)}</span>
      <span class="tb-sep" aria-hidden="true"></span>
      <!--
      ...
      {:else if uiStore.currentTool === 'fixed'}
        <span class="tb-command-hint">{t('sketch.fixedHint')}</span>
      {:else if uiStore.currentTool === 'dimension'}
        <ToolDimensionOptions />
      {:else if uiStore.currentTool === 'support'}
        <ToolSupportOptions />
      {:else if uiStore.currentTool === 'load'}
        <ToolLoadOptions />
      {:else if uiStore.currentTool === 'cylinder'}
        <span class="tb-command-hint">점 두 개를 차례로 선택</span>
        <label class="tb-input"><span>F:</span><input type="number" step="any" value={toDisplay(uiStore.cylinderForce, 'force', uiStore.unitSystem)}
          oninput={(e) => uiStore.cylinderForce = fromDisplay(parseFloat(e.currentTarget.value), 'force', uiStore.unitSystem)} /><span>{unitLabel('force', uiStore.unitSystem)}</span></label>
        <span class="tb-command-hint">+ 밀어냄 · − 당김</span>
...
<style>
  ...
  */
  ...
  .tb-input { display:flex; align-items:center; gap:4px; white-space:nowrap; }
  .tb-input input { width:72px; padding:2px 4px; color:var(--st-text); background:var(--st-surface-2); border:1px solid var(--st-hair-strong); border-radius:3px; }
