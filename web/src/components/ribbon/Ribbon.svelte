<script lang="ts">
  ...
  const solved = $derived(resultsStore.results != null);
  ...
  const diagramCmds = $derived.by((): Cmd[] => {
    ...
    const cmds: Cmd[] = [
      ...
      { id: 'momentY', icon: 'moment', label: F2D.moment, nameKey: 'ribbon.nameMomentY', panel: 'results', diagram: 'moment', enabled: any },
      { id: 'shearZ', icon: 'shear', label: F2D.shear, nameKey: 'ribbon.nameShearZ', panel: 'results', diagram: 'shear', enabled: any },
    ];
    /*
  ...
  const GROUPS: Group[] = $derived([
    {
      ...
      cmds: [
        /*
         * Selection opens its kind filter and also arms the Select pointer.
         * Treating this as configuration alone left whichever edit tool was
         * active (node, element, support or load) mutating the model after the
         * user had explicitly chosen Selection.
         *
         * Panning is a middle-button viewport gesture, so this command only
         * needs to arm selection and show the selection panel.
         */
        { id: 'select', icon: 'select', labelKey: 'ribbon.selection', tool: 'select' },
        /*
    ...
    },
    {
      ...
      cmds: [
        ...
        { id: 'fixed', icon: 'fixed-sketch', labelKey: 'sketch.fixed', tool: 'fixed' },
        { id: 'dimension', icon: 'dimension', labelKey: 'sketch.dimension', tool: 'dimension' },
        {
          id: 'blockCreate',
          icon: 'block-create',
          labelKey: 'ribbon.blockCreate',
          nameKey: 'ribbon.nameBlockCreate',
          enabled: () => modelStore.editingBlockId === null,
          action: () => blockUI.begin('create'),
        },
        {
          id: 'blockMove',
          icon: 'block-move',
          labelKey: 'ribbon.blockMove',
          nameKey: 'ribbon.nameBlockMove',
          enabled: () => modelStore.editingBlockId === null,
          action: () => blockUI.begin('move'),
        },
        {
          id: 'blockRotate',
          icon: 'block-rotate',
          labelKey: 'ribbon.blockRotate',
          nameKey: 'ribbon.nameBlockRotate',
          enabled: () => modelStore.editingBlockId === null,
          action: () => blockUI.begin('rotate'),
        },
      ],
    },
    {
      ...
      cmds: [
        ...
        { id: 'cylinder', icon: 'cylinder', label: '실린더', tool: 'cylinder', panel: 'data', dataTab: 'loads' },
      ],
    },
    {
      /*
      ...
      cmds: [
        { id: 'materials', icon: 'material', labelKey: 'data.materials', panel: 'data', dataTab: 'materials' },
        { id: 'sections', icon: 'section', labelKey: 'data.sections', panel: 'data', dataTab: 'sections' },
      ],
    },
    {
      id: 'analyse',
      ...
      cmds: [
         ...
         */
        { id: 'solve', icon: 'solve', labelKey: 'results.solve', panel: 'results',
          action: () => {
            runSolve();
   ...
   *
   * The pointer modes are not here at all any more: panning is a middle-button
   * viewport gesture, while selection is the only view command.
   */
  /*
   ...
   * drifts. There is no pan command: panning is a middle-button gesture, and
   * selection is the only view command.
   */

  function run(cmd: Cmd) {
    if (cmd.enabled && !cmd.enabled()) return;
    const nextBlockKind = cmd.id === 'blockCreate' ? 'create'
      : cmd.id === 'blockMove' ? 'move'
        : cmd.id === 'blockRotate' ? 'rotate' : null;
    if (blockUI.command && blockUI.command.kind !== nextBlockKind) {
      blockUI.cancel();
    }
    if (cmd.tool) {
      armTool(cmd.tool);
  ...
  function isActive(cmd: Cmd): boolean {
    if (cmd.id === 'blockCreate') return blockUI.command?.kind === 'create';
    if (cmd.id === 'blockMove') return blockUI.command?.kind === 'move';
    if (cmd.id === 'blockRotate') return blockUI.command?.kind === 'rotate';
    // Block/create transform commands temporarily use the Select tool for
    // viewport picking, but they are themselves the active tool.  Do not make
    // that implementation detail look like a second armed ribbon command.
    if (blockUI.command) return false;
    if (cmd.id === 'select') return uiStore.currentTool === 'select';
    if (cmd.tool) return (!cmd.dataTab || activePanel === 'data') && uiStore.currentTool === cmd.tool;
    /*
  ...
  function cmdTitle(c: Cmd, enabled: boolean): string {
    ...
    return `${withKey} — ${t('ribbon.needsSolve')}`;
  }
</script>
      ...
      <section class="rb-group" data-group={g.id} aria-label={t(g.labelKey)}>
        <div class="rb-cmds">
          {#each g.cmds as c (c.id)}
            {@const on = (!c.enabled || c.enabled()) && !(modelStore.editingBlockId !== null && ['support', 'load', 'cylinder', 'solve', 'advanced'].includes(c.id))}
            <button
              class="rb-cmd"
...
<style>
  ...
  */
  .rb-cmd:disabled { opacity: 0.5; cursor: default; }
  ...
  */
  ...
  .rb-quick-btn:disabled { opacity: 0.5; cursor: default; }
