...
>
  ...
  {:else if name === 'visibility' || name === 'visibility-off'}
    <!-- Visibility: the same eye silhouette in both states; hidden adds a
         slash so colour is never the only signal. -->
  ...
  {:else if name === 'block' || name === 'block-create'}
    <!-- Block create: dashed container with block box and plus -->
    <rect x="3" y="3" width="18" height="18" rx="2" stroke-dasharray="3 2" />
    <rect x="7" y="7" width="10" height="10" rx="1" fill="currentColor" fill-opacity="0.15" />
    <path d="M12 9v6M9 12h6" />
  {:else if name === 'move' || name === 'block-move'}
  ...
  {:else if name === 'rotate' || name === 'block-rotate'}
  ...
  {:else if name === 'fixed-sketch'}
    <!-- Sketch fixation: anchored point/line, distinct from a structural support. -->
    <path d="M5 12h14M7 9v6M17 9v6" />
    <path d="M4 18h16M7 18l-2 3M12 18l-2 3M17 18l-2 3" />
  {:else if name === 'dimension'}
    <!-- Dimension line with extension lines and opposed arrowheads. -->
    <path d="M5 6v12M19 6v12M5 12h14" />
    <path d="M5 12l3-2M5 12l3 2M19 12l-3-2M19 12l-3 2" />
  ...
  {:else if name === 'cylinder'}
    ...
    <path d="M5.5 8.5h8v7h-8z" />
    <path d="M13.5 10h5v4h-5" />
  {:else if name === 'solve'}
