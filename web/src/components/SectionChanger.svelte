<script lang="ts">
  ...
  const setAmorph = (field: 'a' | 'iy' | 'iz' | 'j', raw: string) => {
    const value = parseFloat(raw);
    if (!Number.isFinite(value)) return;
    const internal = fromDisplay(value, field === 'a' ? 'area' : 'inertia', us);
    if (field === 'a') amorphA = internal;
    else if (field === 'iy') amorphIy = internal;
    else if (field === 'iz') amorphIz = internal;
    else amorphJ = internal;
  };
  const shownSection = (value: number, quantity: Quantity) => toDisplay(value, quantity, us).toPrecision(4);
        ...
        <div class="sc-body sc-shape-body">
          ...
          <div class="param-grid">
            ...
            <label class="param-field">
              ...
              <div class="param-input">
                <input type="number" step="any" value={toDisplay(amorphA, 'area', us)} oninput={(e) => setAmorph('a', e.currentTarget.value)} />
                <span class="param-unit">{unitLabel('area', us)}</span>
              </div>
            </label>
            <label class="param-field">
              <span>{t('field.iyHoriz')}</span>
              <div class="param-input">
                <input type="number" step="any" value={toDisplay(amorphIy, 'inertia', us)} oninput={(e) => setAmorph('iy', e.currentTarget.value)} />
                <span class="param-unit">{unitLabel('inertia', us)}</span>
              </div>
            </label>
            <label class="param-field">
              <span>{t('field.izVert')}</span>
              <div class="param-input">
                <input type="number" step="any" value={toDisplay(amorphIz, 'inertia', us)} oninput={(e) => setAmorph('iz', e.currentTarget.value)} />
                <span class="param-unit">{unitLabel('inertia', us)}</span>
              </div>
            </label>
            {#if is3D}
              <label class="param-field">
                ...
                <div class="param-input">
                  <input type="number" step="any" value={toDisplay(amorphJ, 'inertia', us)} oninput={(e) => setAmorph('j', e.currentTarget.value)} />
                  <span class="param-unit">{unitLabel('inertia', us)}</span>
                </div>
              </label>
            {/if}
          ...
          {#if amorphValid}
            <div class="results-box">
              ...
              <div class="result-row"><span>A =</span><span class="result-val">{shownSection(amorphA, 'area')} {unitLabel('area', us)}</span></div>
              <div class="result-row"><span>Iy =</span><span class="result-val">{shownSection(amorphIy, 'inertia')} {unitLabel('inertia', us)}</span></div>
              <div class="result-row"><span>Iz =</span><span class="result-val">{shownSection(amorphIz, 'inertia')} {unitLabel('inertia', us)}</span></div>
              {#if is3D}
                <div class="result-row"><span>J =</span><span class="result-val">{shownSection(amorphJ, 'inertia')} {unitLabel('inertia', us)}</span></div>
              {/if}
            </div>
        ...
        <div class="sc-body sc-shape-body">
            ...
            {#each shapeDef.params as p}
              <label class="param-field">
                ...
                <div class="param-input">
                  <input
                    type="number"
                    step={toDisplay(p.step, 'length', us)}
                    value={toDisplay(paramValues[p.id] ?? p.defaultValue, 'length', us)}
                    oninput={(e) => {
                      const v = parseFloat(e.currentTarget.value);
                      if (!isNaN(v)) paramValues = { ...paramValues, [p.id]: fromDisplay(v, 'length', us) };
                    }}
                  />
                  <span class="param-unit">{unitLabel('length', us)}</span>
                </div>
              </label>
            {/each}
          </div>
          ...
          {#if computed}
            <div class="results-box">
              ...
              <div class="result-row"><span>A =</span><span class="result-val">{shownSection(computed.a, 'area')} {unitLabel('area', us)}</span></div>
              <div class="result-row"><span>Iz =</span><span class="result-val">{shownSection(computed.iz, 'inertia')} {unitLabel('inertia', us)}</span></div>
            </div>
...
<style>
   ...
   * painted the DRAWING with `rgba(15, 52, 96, 0.5)` — a pale blue, hardcoded
