<script lang="ts">
  ...
  function syncDataTabWithTool(toolId: string) {
    ...
    if (idx === undefined) return; // select/influenceLine own no tab
  ...
  function handleCopy() {
    ...
    for (const id of nodeIds) {
      ...
      if (n) nodes.push({ origId: n.id, x: n.x, y: n.y, z: 0 });
    }
  ...
  function handlePaste() {
    ...
    // Basic is planar: paste one unit away from the source geometry.
    const ox = 1;
    const oy = 1;
    ...
    modelStore.batch(() => {
      ...
      for (const n of clip.nodes) {
        const newId = modelStore.addNode(n.x + ox, n.y + oy);
  ...
  function handleKeydown(e: KeyboardEvent) {
    ...
    if (!['INPUT', 'TEXTAREA', 'SELECT'].includes(target?.tagName) && !target?.isContentEditable) {
      if (blockUI.command) {
        if (e.key === 'Escape') { blockUI.cancel(); e.preventDefault(); return; }
        const requestedTool = !e.ctrlKey && !e.metaKey
          ? tools.find(tool => tool.key === e.key.toUpperCase())
          : undefined;
        if (requestedTool) blockUI.cancel();
    ...
    if (resultsStore.results) {
      switch (e.key) {
        ...
        case '2': resultsStore.diagramType = 'shear'; return;
        case '3': resultsStore.diagramType = 'moment'; return;
    ...
    // Delete selected dimensions/supports/nodes/elements/loads
    if (e.key === 'Delete' || e.key === 'Backspace') {
      if (uiStore.selectedDimensionId !== null) {
        modelStore.removeDimension(uiStore.selectedDimensionId);
      ...
      if (uiStore.selectedLoads.size > 0) {
        ...
        // Delete strictly from the explicit node/member selections.
        const targets = resolveDeleteTargets(
          { nodes: uiStore.selectedNodes, elements: uiStore.selectedElements, shells: new Set() },
