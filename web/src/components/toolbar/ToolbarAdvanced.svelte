<script lang="ts">
  ...
  import { solvePDelta, solveBuckling, solveModal, solvePlastic } from '../../lib/engine/wasm-solver';
   ...
   * is undefined and all of them collapsed to their generic fallback. That
   * turned a specific solver complaint into the word "Buckling error",
   ...
   * not, so the ✕ on P-Δ, Pcr or Dynamic cleared the wrong slot, the header
   * came straight back and there was no way out of the analysis at all.
   *
  ...
  const ADV: Adv[] = [
    ...
    { key: 'envelope', labelKey: 'advanced.envelope',
      ...
      close: () => { resultsStore.activeView = 'single'; } },
...
<div class="toolbar-section" class:flat data-tour="advanced-section">
  ...
  <div class="advanced-grid">
    {#if shown('kinematic')}
    <!--
      Advanced analyses stay visible in the Basic panel so their controls are
      discoverable and consistent across desktop and mobile layouts.
    -->
    ...
    {/if}
    {#if shown('despiece')}
    <!-- Despiece / member free-body view (solver-free overlay) -->
      {#if !flat || active?.key !== 'despiece'}
    <div class="adv-btn-wrap" style="grid-column: span 2">
      <button class="adv-btn" style="flex:1"
        class:active={resultsStore.diagramType === 'despiece'}
        onclick={() => {
          const hasRes = resultsStore.results;
    ...
    <div class="adv-btn-wrap" style="grid-column: span 2">
      <button class="adv-btn" style="flex:1"
        ...
        onclick={() => {
          ...
          if (!resultsStore.results) { uiStore.toast(t('advanced.calculateFirst'), 'error'); return; }
    ...
    <!--
      ...
      The spectral entry point remains outside the Basic panel until a
      user-defined spectrum can be configured here.
    -->
    ...
    <!--
      ...
      under a greyed-out one, offering to configure a run that cannot start.
    -->
    {#if (flat && active?.key === 'trainLoad') || (!flat && showTrainPanel)}
  ...
  {@const pdR = resultsStore.pdeltaResult}
  {@const moR = resultsStore.modalResult}
  {@const buR = resultsStore.bucklingResult}
  {#if pdR}
  ...
  {#if moR}
    <div class="adv-result-row">
      ...
      <button class="small-btn" onclick={() => { if (resultsStore.activeModeIndex > 0) resultsStore.activeModeIndex--; }} disabled={resultsStore.activeModeIndex === 0}>&#99666680;</button>
      <span class="adv-result-label">{resultsStore.activeModeIndex + 1}/{moR.modes.length}</span>
      <button class="small-btn" onclick={() => { if (moR && resultsStore.activeModeIndex < moR.modes.length - 1) resultsStore.activeModeIndex++; }} disabled={!moR || resultsStore.activeModeIndex >= moR.modes.length - 1}>&#99665580;</button>
    </div>
  ...
  {#if buR}
    <div class="adv-result-row">
      ...
      <button class="small-btn" onclick={() => { if (resultsStore.activeBucklingMode > 0) resultsStore.activeBucklingMode--; }} disabled={resultsStore.activeBucklingMode === 0}>&#99666680;</button>
      <span class="adv-result-label">{resultsStore.activeBucklingMode + 1}/{buR.modes.length}</span>
      <button class="small-btn" onclick={() => { if (buR && resultsStore.activeBucklingMode < buR.modes.length - 1) resultsStore.activeBucklingMode++; }} disabled={!buR || resultsStore.activeBucklingMode >= buR.modes.length - 1}>&#99665580;</button>
    </div>
    <div class="adv-result-info">
  ...
  {#if resultsStore.plasticResult}
    <div class="adv-result-row">
      ...
      <button class="small-btn" onclick={() => { if (resultsStore.plasticStep > 0) resultsStore.plasticStep--; }} disabled={resultsStore.plasticStep === 0}>&#99666680;</button>
      <span class="adv-result-label">{resultsStore.plasticStep + 1}/{resultsStore.plasticResult.steps.length}</span>
      <button class="small-btn" onclick={() => { if (resultsStore.plasticResult && resultsStore.plasticStep < resultsStore.plasticResult.steps.length - 1) resultsStore.plasticStep++; }} disabled={!resultsStore.plasticResult || resultsStore.plasticStep >= resultsStore.plasticResult.steps.length - 1}>&#99665580;</button>
    </div>
    <div class="adv-result-info">
  ...
  {#if resultsStore.movingLoadEnvelope}
    <div class="adv-result-row">
      ...
      <button class="small-btn" onclick={() => { if (resultsStore.activeMovingLoadPosition > 0) { resultsStore.activeMovingLoadPosition--; resultsStore.movingLoadShowEnvelope = false; } }} disabled={resultsStore.activeMovingLoadPosition === 0}>&#99666680;</button>
