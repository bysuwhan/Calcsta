<script lang="ts">
  ...
  import { generateShareURL, loadFromShareLink, shareLinkContainsLegacyEducationMode, isValidBasic2DShareLink, MAX_URL_SAFE } from '../../lib/utils/url-sharing';
  ...
  async function handlePasteShareLink() {
    try {
      ...
      if (shareLinkContainsLegacyEducationMode(text)) {
        uiStore.toast(t('file.unsupportedLegacyMode'), 'error');
        return;
      }
      if (!isValidBasic2DShareLink(text)) {
  ...
  {#if flat || showProjectExtras}
    <div class="sub-section-content">
      ...
      <div class="file-grid">
        ...
        <button class="file-btn" onclick={downloadDXF} title={t('project.exportDxfTooltip')}>
          DXF
        </button>
        <button class="file-btn" onclick={downloadSVG} title={t('project.exportSvgTooltip')}>
          SVG
        </button>
      ...
      <div class="file-grid">
        ...
        <button class="file-btn" onclick={() => window.dispatchEvent(new Event('stabileo-import-dxf'))} title={t('project.openDxfTooltip')}>
          {t('project.openDxf')}
        </button>
