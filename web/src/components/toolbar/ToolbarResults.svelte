<script lang="ts">
  ...
  // ─── Contextual tooltips (subset used by Results) ──────────────
...
<!--
  ...
  to agree about readiness and disabled state forever.
-->
{#if !flat}
<div class="toolbar-section">
  {#if !flat}<h3>{t('results.solve')}</h3>{/if}
  <button class="solve-btn" data-tour="calcular-btn" class:ready={modelReady} onclick={handleSolve} use:tooltip={'solve'}>
    {t('results.solve')}
  </button>
</div>
{/if}
...
<div class="toolbar-section" data-tour="results-section">
  ...
  {#if showResultsPanel || flat}
    {#if resultsStore.results || resultsStore.influenceLine}
      {#if !hideDiagrams}
      ...
      {#if resultsStore.diagramType === 'deformed'}
        <div class="input-group">
          ...
          <button class="scale-step-btn" onclick={() => resultsStore.deformedScale = resultsStore.deformedScale <= 1 ? (resultsStore.deformedScale > 0.5 ? 0.5 : 0.25) : Math.max(1, resultsStore.deformedScale - (resultsStore.deformedScale <= 10 ? 1 : resultsStore.deformedScale <= 100 ? 5 : 50))} title={t('results.decreaseScale')}>◀</button>
          <input type="range" min="0.25" max="1000" step="0.25" bind:value={resultsStore.deformedScale} style="width: 80px" />
          <button class="scale-step-btn" onclick={() => resultsStore.deformedScale = resultsStore.deformedScale < 1 ? (resultsStore.deformedScale < 0.5 ? 0.5 : 1) : Math.min(1000, resultsStore.deformedScale + (resultsStore.deformedScale < 10 ? 1 : resultsStore.deformedScale < 100 ? 5 : 50))} title={t('results.increaseScale')}>▶</button>
          <span style="font-size: 0.7rem; color: #888">{resultsStore.deformedScale}×</span>
        </div>
      ...
      {#if resultsStore.hasCombinations && (showsPrimary || showsSecondary)
        ...
        {@const comboKeys = [...resultsStore.perCombo.keys()]}
        {@const hasEnvelope = resultsStore.fullEnvelope !== null}
              ...
              <div class="input-group">
                ...
                <select
                  ...
                  onchange={(e) => {
                    ...
                    const clearOverlay = () => { resultsStore.setOverlay(null); };
                    if (val === 'single') {
              ...
              {#if showsSecondary}
                <div class="input-group">
                  ...
                  <select onchange={(e) => {
                    const val = (e.target as HTMLSelectElement).value;
                    if (val === 'none') {
                      resultsStore.setOverlay(null);
                    } else if (val === 'single') {
                      resultsStore.setOverlay(resultsStore.singleResults, t('results.simpleLoads'));
                    } else if (val === 'envelope') {
                      resultsStore.setOverlay(resultsStore.fullEnvelope?.maxAbsResults ?? null, t('results.envelope'));
                    } else if (val.startsWith('case_')) {
...
<style>
  ...
  @keyframes gentle-pulse {
    ...
    50% {
      box-shadow: 0 0 8px 2px rgba(233, 69, 96, 0.5);
    }
  }
