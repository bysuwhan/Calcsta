<script lang="ts">
  ...
  // Existing SI presentation remains cm-based; SI_MM and Imperial use the
  // central display boundary while the model stays in m²/m⁴.
  const M2_TO_CM2 = 1e4;   // m² → cm²
  const M4_TO_CM4 = 1e8;   // m⁴ → cm⁴
  const sectionLabel = (q: 'area' | 'inertia') => uiStore.unitSystem === 'SI'
    ? (q === 'area' ? 'cm²' : 'cm⁴') : unitLabel(q, uiStore.unitSystem);
  const sectionDisplay = (v: number, q: 'area' | 'inertia') => uiStore.unitSystem === 'SI'
    ? v * (q === 'area' ? M2_TO_CM2 : M4_TO_CM4)
    : toDisplay(v, q, uiStore.unitSystem);
  const sectionCanonical = (v: number, q: 'area' | 'inertia') => uiStore.unitSystem === 'SI'
    ? v / (q === 'area' ? M2_TO_CM2 : M4_TO_CM4)
    : fromDisplay(v, q, uiStore.unitSystem);
  function fmtA(v: number) { return sectionDisplay(v, 'area').toPrecision(6); }
  function fmtI(v: number) { return sectionDisplay(v, 'inertia').toPrecision(6); }
  ...
  function updateSectionField(id: number, field: string, val: string) {
    if (field === 'name') {
      ...
      if (isNaN(num)) return;
      let modelVal = num;
      if (field === 'a') modelVal = sectionCanonical(num, 'area');
      else if (field === 'iy' || field === 'iz' || field === 'j') modelVal = sectionCanonical(num, 'inertia');
            ...
            <div class="sec-detail">
              ...
              <div class="sec-props">
                {#if derived}
                  ...
                  <div class="prop"><span>A</span><span title={t('table.derivedFromGeometry')}>{fmtA(props.a)} {sectionLabel('area')}</span></div>
                  <div class="prop"><span>Iy</span><span title={t('table.derivedFromGeometry')}>{fmtI(props.iy ?? props.iz)} {sectionLabel('inertia')}</span></div>
                  <div class="prop"><span>Iz</span><span title={t('table.derivedFromGeometry')}>{fmtI(props.iz)} {sectionLabel('inertia')}</span></div>
                {:else}
                  <label class="prop"><span>A ({sectionLabel('area')})</span><input type="number" step="any" value={sectionDisplay(sec.a, 'area')} onchange={(e) => updateSectionField(sec.id, 'a', e.currentTarget.value)} /></label>
                  <label class="prop"><span>Iy ({sectionLabel('inertia')})</span><input type="number" step="any" value={sectionDisplay(sec.iy ?? sec.iz, 'inertia')} onchange={(e) => updateSectionField(sec.id, 'iy', e.currentTarget.value)} /></label>
                  <label class="prop"><span>Iz ({sectionLabel('inertia')})</span><input type="number" step="any" value={sectionDisplay(sec.iz, 'inertia')} onchange={(e) => updateSectionField(sec.id, 'iz', e.currentTarget.value)} /></label>
                {/if}
