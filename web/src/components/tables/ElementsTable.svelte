...
<table>
  <thead>
    <tr><th>ID</th><th>{t('table.type')}</th><th>{t('table.nodeI')}</th><th>{t('table.nodeJ')}</th><th>{t('prop.material')}</th><th>{t('table.sectionHeader')}</th><th>L ({unitLabel('length', uiStore.unitSystem)})</th><th></th></tr>
  </thead>
  <tbody>
    {#each elementsArr as elem}
      <tr>
        ...
        <td>{elem.nodeJ}</td>
        <td>
          <select disabled={!modelStore.canEditElement(elem.id)} value={String(elem.materialId)} onchange={(e) => changeElementMaterial(elem.id, e.currentTarget.value)}>
        ...
        <td>{toDisplay(modelStore.getElementLength(elem.id), 'length', uiStore.unitSystem).toFixed(3)}</td>
