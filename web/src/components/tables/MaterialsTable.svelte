<script lang="ts">
  ...
  const quantityFor = (field: string): Quantity | null =>
    field === 'e' || field === 'fy' ? 'stress' : field === 'rho' ? 'density' : null;
  ...
  function updateMaterialField(id: number, field: string, val: string) {
    if (field === 'name') {
      ...
      if (isNaN(num)) return;
      const quantity = quantityFor(field);
      modelStore.updateMaterial(id, { [field]: quantity ? fromDisplay(num, quantity, us) : num });
      resultsStore.clear();
    }
  }
        ...
        <tr class="detail-row">
          <td colspan="3">
            <div class="mat-detail">
              <label class="prop"><span>E ({unitLabel('stress', us)})</span><input type="number" step="any" value={toDisplay(mat.e, 'stress', us)} onchange={(e) => updateMaterialField(mat.id, 'e', e.currentTarget.value)} /></label>
              ...
              <label class="prop"><span>ρ ({unitLabel('density', us)})</span><input type="number" step="any" value={toDisplay(mat.rho, 'density', us)} onchange={(e) => updateMaterialField(mat.id, 'rho', e.currentTarget.value)} /></label>
              <label class="prop"><span>fy ({unitLabel('stress', us)})</span><input type="number" step="any" value={mat.fy == null ? '' : toDisplay(mat.fy, 'stress', us)} onchange={(e) => updateMaterialField(mat.id, 'fy', e.currentTarget.value)} /></label>
            </div>
          </td>
        </tr>
      {/if}
