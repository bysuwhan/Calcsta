<script lang="ts">
  ...
  const shown = (value: number, quantity: Quantity) => toDisplay(value, quantity, uiStore.unitSystem);
  const entered = (value: string, quantity: Quantity) => fromDisplay(parseFloat(value), quantity, uiStore.unitSystem);
...
<!-- 2D support types -->
  ...
  {#if uiStore.supportType === 'spring'}
    ...
    <label class="ft-input-group">
      <span>kx:</span>
      <input type="number" value={shown(uiStore.springKx, 'springK')} step="any" oninput={(e) => uiStore.springKx = entered(e.currentTarget.value, 'springK')} />
      <span class="ft-unit">{unitLabel('springK', uiStore.unitSystem)}</span>
    </label>
    <label class="ft-input-group">
      <span>ky:</span>
      <input type="number" value={shown(uiStore.springKy, 'springK')} step="any" oninput={(e) => uiStore.springKy = entered(e.currentTarget.value, 'springK')} />
      <span class="ft-unit">{unitLabel('springK', uiStore.unitSystem)}</span>
    </label>
    <label class="ft-input-group">
      <span>kθ:</span>
      <input type="number" value={shown(uiStore.springKz, 'springKr')} step="any" oninput={(e) => uiStore.springKz = entered(e.currentTarget.value, 'springKr')} />
      <span class="ft-unit">{unitLabel('springKr', uiStore.unitSystem)}</span>
    </label>
    <span class="ft-sep">|</span>
  ...
  {:else if uiStore.supportType === 'roller'}
    ...
    <label class="ft-input-group" title={t('float.prescribedRollerDisp')}>
      <span>di:</span>
      <input type="number" value={shown(uiStore.supportDx, 'displacement')} step="0.001" oninput={(e) => uiStore.supportDx = entered(e.currentTarget.value, 'displacement')} />
      <span class="ft-unit">{unitLabel('displacement', uiStore.unitSystem)}</span>
    </label>
    <label class="ft-input-group" title={t('float.supportAngle')}>
      <span>α:</span>
      <input type="number" bind:value={uiStore.supportAngle} step="5" />
  ...
  {:else}
    ...
    {#if uiStore.supportType === 'fixed' || uiStore.supportType === 'pinned'}
      ...
      <label class="ft-input-group" title={t('float.prescribedDy')}>
        <span>dy:</span>
        <input type="number" value={shown(uiStore.supportDy, 'displacement')} step="0.001" oninput={(e) => uiStore.supportDy = entered(e.currentTarget.value, 'displacement')} />
      </label>
    {/if}
