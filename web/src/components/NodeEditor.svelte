<script lang="ts">
  ...
  import { worldToLocal, localToWorld } from '../lib/model/blocks';
  ...
  const block = $derived(modelStore.blocks.instances.find(b => b.id === modelStore.editingBlockId));
  const position = $derived(node && block ? worldToLocal(node, block) : node);
  ...
  $effect(() => {
    if (node) {
      localX = String(toDisplay(position!.x, 'length', us));
      localY = String(toDisplay(position!.y, 'length', us));
  ...
  function confirm() {
    ...
    const x = fromDisplay(parseFloat(localX), 'length', us);
    const y = fromDisplay(parseFloat(localY), 'length', us);
    if (isNaN(x) || isNaN(y)) return;
    const world = block ? localToWorld({ x, y }, block) : { x, y };
    if (world.x !== node.x || world.y !== node.y) {
      historyStore.pushState();
      modelStore.updateNode(nodeId, world.x, world.y);
      resultsStore.clear();
    }
    close();
  }
...
{#if node}
  ...
  <div class="editor" style="left: {pos.x}px; top: {pos.y}px;">
    ...
    <div class="field">
      <span>{TWO_D_HORIZONTAL_AXIS_LABEL} ({lengthUnit}):</span>
      <input
        bind:this={inputX}
        type="number"
        step={lengthStep}
    ...
    <div class="field">
      <span>{TWO_D_VERTICAL_AXIS_LABEL} ({lengthUnit}):</span>
      <input
        bind:this={inputY}
        type="number"
        step={lengthStep}
