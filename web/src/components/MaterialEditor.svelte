<script lang="ts">
  ...
  $effect(() => {
    if (mat) {
      localName = mat.name;
      localE = String(toDisplay(mat.e, 'stress', us));
      localNu = String(mat.nu);
      localRho = String(toDisplay(mat.rho, 'density', us));
      localFy = mat.fy != null ? String(toDisplay(mat.fy, 'stress', us)) : '';
  ...
  function confirm() {
    ...
    modelStore.updateMaterial(matId, {
      name: localName,
      e: fromDisplay(e, 'stress', us),
      nu,
      rho: fromDisplay(rho, 'density', us),
      fy: isNaN(fy) ? undefined : fromDisplay(fy, 'stress', us),
    });
    close();
  }
...
{#if mat}
  ...
  <div class="editor">
    ...
    <div class="field">
      <span>E ({unitLabel('stress', us)}):</span>
      <input
        type="number"
        step="1000"
        bind:value={localE}
    ...
    <div class="field">
      <span>ρ ({unitLabel('density', us)}):</span>
      <input
        type="number"
        step="0.1"
        bind:value={localRho}
    ...
    <div class="field">
      <span>fy ({unitLabel('stress', us)}):</span>
      <input
        type="number"
        step="10"
        bind:value={localFy}
