<script lang="ts">
  ...
  function updateLoadField(loadId: number, field: string, val: string, quantity: Quantity) {
    ...
    if (isNaN(num)) return;
    modelStore.updateLoad(loadId, { [field]: fromDisplay(num, quantity, us) });
  }
</script>
...
<div class="panel-section">
  ...
  {#each uiStore.selectedNodes as nodeId}
    {#if modelStore.getNode(nodeId)}
      ...
      <div class="property-row">
        <span>{TWO_D_VERTICAL_AXIS_LABEL}:</span>
        <span>{dv(get2DDisplayedVertical(node), 'length').toFixed(3)} {ul('length')}</span>
      </div>
      ...
      {#if nodalLoads.length > 0}
        ...
        {#each nodalLoads as nl}
          <div class="property-row">
            <span>Fx:</span>
            <input type="number" step="any" value={dv(nl.fx, 'force')} class="prop-input" onchange={(e) => updateLoadField(nl.id, 'fx', e.currentTarget.value, 'force')} />
            <span>{ul('force')}</span>
          </div>
          <div class="property-row">
            <span>{TWO_D_NODAL_LOAD_LABELS.vertical}:</span>
            <input type="number" step="any" value={dv(get2DDisplayNodalLoadVertical(nl), 'force')} class="prop-input" onchange={(e) => updateLoadField(nl.id, 'fz', e.currentTarget.value, 'force')} />
            <span>{ul('force')}</span>
          </div>
          <div class="property-row">
            <span>{TWO_D_NODAL_LOAD_LABELS.moment}:</span>
            <input type="number" step="any" value={dv(get2DDisplayNodalLoadMoment(nl), 'moment')} class="prop-input" onchange={(e) => updateLoadField(nl.id, 'my', e.currentTarget.value, 'moment')} />
            <span>{ul('moment')}</span>
          </div>
        {/each}
      {/if}
