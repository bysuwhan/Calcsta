<script lang="ts">
    ...
    debounceTimer = setTimeout(() => {
      ...
      for (let i = 0; i < loads.length; i++) {
        ...
        if (l.type === 'nodal') {
          ...
          const baseLoads = baseline!.loads[i]?.data as { force?: number };
          if (baseLoads?.force !== undefined) l.data.force = baseLoads.force * f;
      ...
      // Re-solve the temporary 2D model.
  ...
  function loadLabel(i: number): string {
    ...
    if (l.type === 'cylinder') return `Cylinder ${l.data.nodeI}–${l.data.nodeJ}: F=${l.data.force}`;
  ...
  <div class="wif-panel" class:docked={docked}>
    ...
    <div class="wif-body">
      ...
      <div class="wif-section">
        ...
        <div class="wif-slider-row">
          ...
          <input
            ...
            oninput={applyAndSolve}
          />
          <span class="wif-val" title="{toDisplay(baselineE * eFactor, 'stress', uiStore.unitSystem).toFixed(3)} {unitLabel('stress', uiStore.unitSystem)}">{eFactor.toFixed(2)}x</span>
        </div>
        <div class="wif-current">E = {formatSci(toDisplay(baselineE * eFactor, 'stress', uiStore.unitSystem))} {unitLabel('stress', uiStore.unitSystem)}</div>
      </div>
      ...
      <div class="wif-section">
        ...
        <div class="wif-current">A = {formatSci(toDisplay(baselineA * aFactor, 'area', uiStore.unitSystem))} {unitLabel('area', uiStore.unitSystem)}</div>
        ...
        <div class="wif-current">Iz = {formatSci(toDisplay(baselineIz * izFactor, 'inertia', uiStore.unitSystem))} {unitLabel('inertia', uiStore.unitSystem)}</div>
      </div>
    </div>
  </div>
{/if}
