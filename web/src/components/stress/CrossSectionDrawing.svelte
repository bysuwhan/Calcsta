    ...
    <svg viewBox="-90 -90 180 180" class="ssp-cross-svg">
      ...
      {#if showCentralCore && centralCore && centralCore.vertices.length >= 3}
        ...
        <polygon
          ...
          fill={showStressMap ? 'none' : 'rgba(255, 140, 0, 0.5)'}
          stroke="var(--st-warn)"
      ...
      {#if showPressureCenter && pressureCenter}
        ...
        {#if pressureCenter.insideCore}
          <circle cx={clampX} cy={clampY} r="8" fill="rgba(42, 168, 105, 0.5)" stroke="var(--st-ok)" stroke-width={0.8 * strokeK} stroke-dasharray="2,1" opacity="0.9" />
        {/if}
        <!-- Crosshair marker -->
      ...
      {#if is3D && analysis3D}
        ...
        {:else}
          ...
          {#if Math.abs(sigmaN3d) > 0.001}
            <text x={xBaseR} y={-rs.h / 2 * sc - 4} fill="var(--st-warn)" font-size={4.5 * textK} text-anchor="start">+ N/A = {fmt(stress(sigmaN3d))} {stressUnit}</text>
          {/if}
        {/if}
        ...
        {#if showShearOnDrawing}
          ...
          {#if maxTauY > 0.01}
            ...
            <text x={xBaseL - 2} y={1} fill="var(--st-accent)" font-size={5 * textK} text-anchor="end">{fmt(stress(maxTauY))} {stressUnit}</text>
          {/if}
        {/if}
        ...
        {:else}
          ...
          <!-- N/A annotation -->
          {#if Math.abs(sigmaN3d) > 0.001}
            <text x={-(rs.b / 2 * sc)} y={yBaseBot + 32} fill="var(--st-warn)" font-size={4.5 * textK} text-anchor="start">+ N/A = {fmt(stress(sigmaN3d))} {stressUnit}</text>
          {/if}
        {/if}
      ...
      {:else if analysis2D}
        ...
        {:else}
          ...
          {#if Math.abs(sigmaN2d) > 0.001}
            <text x={xBase2d} y={-rs2d.h / 2 * sc2d - 10} fill="var(--st-warn)" font-size={4.5 * textK} text-anchor="start">+ N/A = {fmt(stress(sigmaN2d))} {stressUnit}</text>
          {/if}
        {/if}
        {/if}<!-- end showSigma (2D) -->
        ...
        {#if showShearOnDrawing && isMassive && analysis2D}
          ...
          >{fmt(stress(maxAbsTau))} {stressUnit}</text>
        {/if}
...
<style>
  ...
  .ssp-ecc-marker.inactive { opacity: 0.5; }
