<script lang="ts">
  ...
  const stress = (value: number) => toDisplay(value, 'stress', uiStore.unitSystem);
  const length = (value: number) => toDisplay(value, 'length', uiStore.unitSystem);
  const stressUnit = $derived(unitLabel('stress', uiStore.unitSystem));
  const lengthUnit = $derived(unitLabel('length', uiStore.unitSystem));
</script>
  ...
  <div class="ssp-stress-detail">
    {#if is3D && analysis3D}
      ...
      <div class="ssp-stress-row">
        ...
        <span class="ssp-stress-val">{fmt(stress(analysis3D.tauTotal))} {stressUnit}</span>
        <span class="ssp-help" title={t('stress.tauTotalHelp')}>?</span>
      </div>
      <div class="ssp-divider"></div>
      <div class="ssp-stress-row">
        <span>&sigma;<sub>vm</sub> =</span>
        <span class="ssp-stress-val">{fmt(stress(analysis3D.failure.vonMises))} {stressUnit}</span>
      ...
      <div class="ssp-stress-row">
        <span>Tresca:</span>
        <span class="ssp-stress-val">&tau;<sub>max</sub> = {fmt(stress(analysis3D.mohr.tauMax))} {stressUnit}</span>
      ...
      <div class="ssp-stress-row">
        <span>Rankine:</span>
        <span class="ssp-stress-val">&sigma;<sub>max</sub> = {fmt(stress(analysis3D.failure.rankine))} {stressUnit}</span>
      ...
      {#if analysis3D.failure.fy}
        ...
        <div class="ssp-fy-legend">
          <span>0</span>
          <span>f<sub>y</sub> = {fmt(stress(analysis3D.failure.fy))} {stressUnit}</span>
        </div>
      {/if}
      <!-- Neutral axis info -->
      {#if analysis3D.neutralAxis.exists}
        ...
        <div class="ssp-stress-row">
          ...
          {#if analysis3D.neutralAxis.slope === Infinity}
            <span class="ssp-stress-val">vertical (z = {fmt(length(analysis3D.neutralAxis.intercept))} {lengthUnit})</span>
          {:else if Math.abs(analysis3D.neutralAxis.slope) < 0.001}
            <span class="ssp-stress-val">horizontal (y = {fmt(length(analysis3D.neutralAxis.intercept))} {lengthUnit})</span>
          {:else}
    ...
    {:else if analysis2D}
      ...
      <div class="ssp-stress-row">
        ...
        <span class="ssp-stress-val" class:tension={analysis2D.sigmaAtY > 0} class:compression={analysis2D.sigmaAtY < 0}>
          {fmt(stress(analysis2D.sigmaAtY))} {stressUnit}
        </span>
        <span class="ssp-help" title={t('stress.sigma2dHelp')}>?</span>
      </div>
      <div class="ssp-stress-row">
        <span>&tau;<sub>xy</sub> =</span>
        <span class="ssp-stress-val">{fmt(stress(analysis2D.tauAtY))} {stressUnit}</span>
      ...
      {#if Math.max(...analysis2D.distribution.map(p => Math.abs(p.tau))) > 0.01}
        ...
        <div class="ssp-stress-row ssp-tau-note">
          ...
          <span class="ssp-stress-val">{fmt(stress(maxAbsTau))} {stressUnit}</span>
      ...
      <div class="ssp-stress-row">
        <span>&sigma;<sub>vm</sub> =</span>
        <span class="ssp-stress-val">{fmt(stress(analysis2D.failure.vonMises))} {stressUnit}</span>
      ...
      <div class="ssp-stress-row">
        <span>Tresca:</span>
        <span class="ssp-stress-val">&tau;<sub>max</sub> = {fmt(stress(analysis2D.mohr.tauMax))} {stressUnit}</span>
      ...
      <div class="ssp-stress-row">
        <span>Rankine:</span>
        <span class="ssp-stress-val">&sigma;<sub>max</sub> = {fmt(stress(analysis2D.failure.rankine))} {stressUnit}</span>
      ...
      {#if analysis2D.failure.fy}
        ...
        <div class="ssp-fy-legend">
          <span>0</span>
          <span>f<sub>y</sub> = {fmt(stress(analysis2D.failure.fy))} {stressUnit}</span>
        </div>
      {/if}
      <!-- Neutral axis in 2D -->
      {#if analysis2D.neutralAxisY !== null}
        ...
        <div class="ssp-stress-row">
          <span>{t('stress.neutralAxis')}</span>
          <span class="ssp-stress-val">y = {fmt(length(analysis2D.neutralAxisY), 3)} {lengthUnit}</span>
