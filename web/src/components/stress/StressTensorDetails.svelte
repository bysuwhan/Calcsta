<script lang="ts">
  ...
  const stressSquared = (value: number) => {
    const factor = toDisplay(1, 'stress', uiStore.unitSystem);
  ...
  {:else}
    <div class="ssp-tensor-block">
      ...
      <div class="ssp-tensor-head">
        <span>{t('stress.tensorStress')}</span><span class="ssp-tensor-unit">{stressUnit}</span>
      </div>
      <div class="ssp-matrix">
        ...
        <div class="ssp-matrix-grid">
          {#each stressRows as row}
            {#each row as v}
              <span class="ssp-cell" class:zero={v === 0}>{fmt(stress(v))}</span>
            {/each}
          {/each}
        </div>
        <span class="ssp-bracket">⎤<br />⎥<br />⎦</span>
      ...
      <div class="ssp-tensor-rows">
        <div class="ssp-trow">
          ...
          <span class="ssp-tval">
            {fmt(stress(tensors.principalStress.values[0]))} / {fmt(stress(tensors.principalStress.values[1]))} / {fmt(stress(tensors.principalStress.values[2]))}
            <span class="ssp-tunit">{stressUnit}</span>
          </span>
        </div>
        <div class="ssp-trow">
        ...
        <div class="ssp-trow">
          ...
          <span class="ssp-tval">{fmt(stress(tensors.principalStress.maxShear))}<span class="ssp-tunit">{stressUnit}</span></span>
        </div>
        <div class="ssp-trow">
          <span class="ssp-tlabel">I<sub>1</sub></span>
          <span class="ssp-tval">{fmt(stress(tensors.invariants.i1))}<span class="ssp-tunit">{stressUnit}</span></span>
        </div>
        <div class="ssp-trow">
          <span class="ssp-tlabel">J<sub>2</sub></span>
          <span class="ssp-tval">{fmt(stressSquared(tensors.invariants.j2))}<span class="ssp-tunit">{stressUnit}²</span></span>
        </div>
        <div class="ssp-trow">
          <span class="ssp-tlabel">&sigma;<sub>hid</sub></span>
          <span class="ssp-tval">{fmt(stress(tensors.invariants.hydrostatic))}<span class="ssp-tunit">{stressUnit}</span></span>
        </div>
        <div class="ssp-trow ssp-trow-vm">
          <span class="ssp-tlabel">&sigma;<sub>vM</sub></span>
          <span class="ssp-tval">
            {fmt(stress(vonMises))}<span class="ssp-tunit">{stressUnit}</span>
            {#if utilisation !== null}
...
<style>
  ...
  .ssp-util {
    ...
    padding: 0 4px;
    border-radius: 3px;
    background: rgba(42, 168, 105, 0.5);
    color: var(--st-ok);
    font-size: 0.9em;
  }
  .ssp-util.over { background: rgba(214, 69, 69, 0.5); color: var(--st-danger); }
  ...
  .ssp-help:hover { opacity: 1; background: rgba(127, 212, 204, 0.5); }
