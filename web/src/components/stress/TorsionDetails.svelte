<script lang="ts">
  ...
  const display = (value: number, quantity: 'moment' | 'stress' | 'inertia' | 'length') => toDisplay(value, quantity, uiStore.unitSystem);
  const momentUnit = $derived(unitLabel('moment', uiStore.unitSystem));
  const stressUnit = $derived(unitLabel('stress', uiStore.unitSystem));
  const inertiaUnit = $derived(unitLabel('inertia', uiStore.unitSystem));
  ...
  {:else}
    <div class="ssp-tor">
      ...
      <div class="ssp-tor-row">
        ...
        <span class="ssp-tor-val">{fmt(display(Math.abs(torque), 'moment'))}<span class="ssp-tor-unit">{momentUnit}</span></span>
      </div>
      <div class="ssp-tor-row ssp-tor-peak">
        <span class="ssp-tor-label">&tau;<sub>max</sub></span>
        <span class="ssp-tor-val">{fmt(display(flow.tauMax, 'stress'))}<span class="ssp-tor-unit">{stressUnit}</span></span>
      </div>
      <div class="ssp-tor-row">
        <span class="ssp-tor-label">J</span>
        <span class="ssp-tor-val">{fmt(display(flow.j, 'inertia'))}<span class="ssp-tor-unit">{inertiaUnit}</span></span>
      </div>
          ...
          <div class="ssp-tt-row" class:na={!th.applies} class:governs={th.governs}>
            ...
            {#if th.applies && th.tauMax !== null}
              <div class="ssp-tt-value">
                <span class="ssp-tt-tau">&tau;<sub>max</sub> = {fmt(display(th.tauMax, 'stress'))}<span class="ssp-tor-unit">{stressUnit}</span></span>
        ...
        <div class="ssp-tor-warp">
          ...
          {#if warp.lambda}
            <div class="ssp-tor-row">
              ...
              <span class="ssp-tor-val">{fmt(display(warp.lambda, 'length'))}<span class="ssp-tor-unit">{lengthUnit}</span></span>
            </div>
            <div class="ssp-tor-row">
          ...
          {#if response}
            ...
            <div class="ssp-tor-row ssp-tor-peak">
              ...
              <span class="ssp-tor-val">{fmt(display(response.sigmaW, 'stress'))}<span class="ssp-tor-unit">{stressUnit}</span></span>
            </div>
