<script lang="ts">
  ...
  const codes = $derived.by(() => {
    if (family) return codesForMode(codesForFamily(family), false).map((c) => ({ id: c.id, name: c.name }));
        ...
        {#each filtered as p}
          ...
          <button class="preset-item" onclick={() => onselect(p)}>
            ...
            <span class="preset-props">
              E={toDisplay(p.e, 'stress', uiStore.unitSystem).toLocaleString()} {unitLabel('stress', uiStore.unitSystem)}
              ...
              {#if p.fy}
                fy={toDisplay(p.fy, 'stress', uiStore.unitSystem).toLocaleString()} {unitLabel('stress', uiStore.unitSystem)}{#if bands}<span class="preset-band" title={bands.full}
                >{bands.tail}</span>{/if}
              {/if}
              {#if p.fu} fu={toDisplay(p.fu, 'stress', uiStore.unitSystem).toLocaleString()} {unitLabel('stress', uiStore.unitSystem)}{/if}
              ρ={toDisplay(p.rho, 'density', uiStore.unitSystem).toLocaleString()} {unitLabel('density', uiStore.unitSystem)}
            </span>
          </button>
        {/each}
