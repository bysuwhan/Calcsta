...
/** Fixed CSS-pixel radius used when the pointer targets an existing node. */
export const NODE_PICK_RADIUS_PX = 10;
/** Fixed CSS-pixel radius used for line-like element targets. */
export const ELEMENT_PICK_RADIUS_PX = 8;
/** Fixed CSS-pixel radius used for support and load targets. */
export const ATTACHMENT_PICK_RADIUS_PX = 10;
...
/**
 * Finds the nearest node in screen space.
 *
 * Unlike `findNearestNode`, this tolerance is measured after projection and
 * zoom, so increasing px/m never inflates the clickable area. `nodes` should
 * already be projected into the active 2D working plane.
 */
export function findNearestNodeScreen(
  screenX: number,
  screenY: number,
  maxDistPx: number,
  ...
  let nearest: NearestNodeResult | null = null;
  let minDistSq = maxDistPx * maxDistPx;

  for (const node of nodes.values()) {
    const point = worldToScreen(node.x, node.y);
    const distSq = (point.x - screenX) ** 2 + (point.y - screenY) ** 2;
    if (distSq <= minDistSq) {
      minDistSq = distSq;
      nearest = node;
    }
...
function distanceToLoad(
  ...
  if (load.type === 'cylinder') {
    const d = load.data;
    const ni = nodes.get(d.nodeI), nj = nodes.get(d.nodeJ);
    if (!ni || !nj) return Infinity;
    const dx = nj.x - ni.x, dy = nj.y - ni.y, lenSq = dx * dx + dy * dy;
    if (lenSq < 1e-10) return Infinity;
    const t = Math.max(0, Math.min(1, ((wx - ni.x) * dx + (wy - ni.y) * dy) / lenSq));
    return Math.hypot(wx - (ni.x + t * dx), wy - (ni.y + t * dy));
  }
