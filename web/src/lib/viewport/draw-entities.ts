...
export function drawElement(
  ...
  // Draw hinges at the joint itself. Connection state is a node property in
  // Basic 2D, so every released member end at a multi-member joint deliberately
  // overdraws the same circle (the same visual used by a two-member joint).
  const dx = sj.x - si.x;
  const dy = sj.y - si.y;
  ...
  if (opts.showLengths) {
    ...
    ctx.fillText(`${toDisplay(opts.worldLength, 'length', opts.unitSystem ?? 'SI').toFixed(3)} ${unitLabel('length', opts.unitSystem ?? 'SI')}`, midX + nx, midY + ny + offset);
    ctx.textAlign = 'left';
  }
}

/** Draw a deliberately simplified actuator rather than a structural member.
 * The end eyes are short plain lines, the left half is the barrel/bore and the
 * right half is the thicker piston rod. Arrowheads encode the actual action:
 * positive pushes the nodes apart, negative pulls them together. */
export function drawCylinderLoad(
  ctx: CanvasRenderingContext2D,
  load: { nodeI: number; nodeJ: number; force: number },
  ni: { x: number; y: number },
  nj: { x: number; y: number },
  worldToScreen: WorldToScreenFn,
  color: string,
  ...
  const a = worldToScreen(ni.x, ni.y), b = worldToScreen(nj.x, nj.y);
  const dx = b.x - a.x, dy = b.y - a.y, len = Math.hypot(dx, dy);
  if (len < 18) return;
  const ux = dx / len, uy = dy / len, nx = -uy, ny = ux;
  const p = (t: number, off = 0) => ({ x: a.x + dx * t + nx * off, y: a.y + dy * t + ny * off });
  const barrelA = Math.min(0.28, 18 / len), barrelB = Math.max(0.55, 0.58 - 8 / len);
  const rodB = Math.max(barrelB + 0.08, 1 - Math.min(0.18, 14 / len));

  ctx.save();
  ctx.strokeStyle = color; ctx.fillStyle = color; ctx.lineCap = 'round';
  // Plain end links.
  ctx.lineWidth = 2.5; ctx.beginPath(); ctx.moveTo(a.x, a.y); ctx.lineTo(p(barrelA).x, p(barrelA).y);
  ctx.moveTo(p(rodB).x, p(rodB).y); ctx.lineTo(b.x, b.y); ctx.stroke();
  // Concave/open bore: parallel walls with an inset curved back at the cap.
  const half = 7;
  const ba1 = p(barrelA, half), ba2 = p(barrelA, -half), bb1 = p(barrelB, half), bb2 = p(barrelB, -half);
  ctx.lineWidth = 2; ctx.beginPath();
  ctx.moveTo(bb1.x, bb1.y); ctx.lineTo(ba1.x, ba1.y);
  ctx.quadraticCurveTo(p(barrelA - 0.035).x, p(barrelA - 0.035).y, ba2.x, ba2.y);
  ctx.lineTo(bb2.x, bb2.y); ctx.stroke();
  // Thick piston rod entering the open bore.
  ctx.lineWidth = 5; ctx.beginPath(); ctx.moveTo(p(barrelB - 0.05).x, p(barrelB - 0.05).y); ctx.lineTo(p(rodB).x, p(rodB).y); ctx.stroke();
  ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(bb1.x, bb1.y); ctx.lineTo(bb2.x, bb2.y); ctx.stroke();

  const push = load.force >= 0;
  const arrow = (at: { x: number; y: number }, dir: number) => {
    const size = 6, tx = at.x + ux * dir * size, ty = at.y + uy * dir * size;
    ctx.beginPath(); ctx.moveTo(tx, ty);
    ctx.lineTo(tx - ux * dir * size + nx * 3.5, ty - uy * dir * size + ny * 3.5);
    ctx.lineTo(tx - ux * dir * size - nx * 3.5, ty - uy * dir * size - ny * 3.5); ctx.closePath(); ctx.fill();
  };
  arrow(p(0.12), push ? -1 : 1); arrow(p(0.88), push ? 1 : -1);
  const mid = p(0.5, -15);
  ctx.font = '11px sans-serif'; ctx.textAlign = 'center';
  ctx.fillText(`${load.force > 0 ? '+' : ''}${toDisplay(load.force, 'force', unitSystem).toFixed(2)} ${unitLabel('force', unitSystem)}`, mid.x, mid.y);
  ctx.restore();
}
...
export function drawNodalLoad(
  ctx: CanvasRenderingContext2D,
  screen: ScreenPoint,
  loadData: { fx: number; fy?: number; fz?: number; mz?: number; my?: number; displayAsVector?: boolean },
  ...
  const vectorMagnitude = Math.hypot(loadData.fx, vertical);

  ctx.strokeStyle = color;
  ctx.fillStyle = color;
  ctx.lineWidth = 2;

  if (loadData.displayAsVector && vectorMagnitude > 0.001) {
    const forceX = loadData.fx / vectorMagnitude;
    const forceY = -vertical / vectorMagnitude;
    const tailX = screen.x - forceX * arrowLen;
    const tailY = screen.y - forceY * arrowLen;
    const perpX = -forceY;
    const perpY = forceX;
    ...
    ctx.lineTo(screen.x - forceX * 10 + perpX * 5, screen.y - forceY * 10 + perpY * 5);
    ctx.lineTo(screen.x - forceX * 10 - perpX * 5, screen.y - forceY * 10 - perpY * 5);
    ctx.closePath();
    ctx.fill();

    labels.block({ x1: screen.x, y1: screen.y, x2: tailX, y2: tailY });
    labels.add({
      text: `${prefix}F=${Math.abs(toDisplay(vectorMagnitude, 'force', unitSystem)).toFixed(3)} ${unitLabel('force', unitSystem)}`,
      colour: color,
      font: '12px sans-serif',
      box: {
        x: tailX - forceX * 6, y: tailY - forceY * 6,
        width: 0, height: 14,
        dirX: -forceX, dirY: -forceY, sweep: 'any',
        anchorX: forceX >= 0 ? 'right' : 'left',
        priority: vectorMagnitude,
      },
    });
  }

  if (!loadData.displayAsVector && Math.abs(vertical) > 0.001) {
    ...
    labels.add({
      text: `${prefix}${TWO_D_NODAL_LOAD_LABELS.vertical}=${Math.abs(toDisplay(vertical, 'force', unitSystem)).toFixed(3)} ${unitLabel('force', unitSystem)}`,
      colour: color,
      font: '12px sans-serif',
      box: {
  ...
  if (!loadData.displayAsVector && Math.abs(loadData.fx) > 0.001) {
    ...
    labels.add({
      text: `${prefix}${TWO_D_NODAL_LOAD_LABELS.horizontal}=${Math.abs(toDisplay(loadData.fx, 'force', unitSystem)).toFixed(3)} ${unitLabel('force', unitSystem)}`,
      colour: color,
      /*
  ...
  if (Math.abs(moment) > 0.001) {
    ...
    labels.add({
      text: `${prefix}${TWO_D_NODAL_LOAD_LABELS.moment}=${Math.abs(toDisplay(moment, 'moment', unitSystem)).toFixed(3)} ${unitLabel('moment', unitSystem)}`,
      colour: color,
      font: '12px sans-serif',
      box: {
...
export function drawConstraintForces(
  ...
  for (const cf of forces) {
    ...
    if (isVertical) {
      ...
      ctx.fillText(`${TWO_D_REACTION_LABELS.vertical}=${Math.abs(toDisplay(cf.force, 'force', unitSystem)).toFixed(2)} ${unitLabel('force', unitSystem)}`, s.x, y1 + dir * 12);
    } else if (cf.dof === 'ux') {
      const dir = cf.force > 0 ? 1 : -1;
      const x1 = s.x - dir * arrowLen;
      ctx.strokeStyle = C; ctx.lineWidth = 2;
      ...
      ctx.closePath(); ctx.fill();
      ctx.font = 'bold 10px sans-serif'; ctx.textAlign = 'center';
      ctx.fillText(`${TWO_D_REACTION_LABELS.horizontal}=${Math.abs(toDisplay(cf.force, 'force', unitSystem)).toFixed(2)} ${unitLabel('force', unitSystem)}`, x1 - dir * 5, s.y - 8);
      ...
      ctx.font = 'bold 10px sans-serif'; ctx.textAlign = 'center';
      ctx.fillText(`${TWO_D_REACTION_LABELS.moment}=${Math.abs(toDisplay(cf.force, 'moment', unitSystem)).toFixed(2)} ${unitLabel('moment', unitSystem)}`, s.x, s.y - radius - 5);
    }
  }
  ctx.textAlign = 'left';
}
