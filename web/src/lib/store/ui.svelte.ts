...
import { setStressUnit, type StressUnit, type UnitSystem } from '../utils/units';
import type { Element3DMetadata } from '../model/element-3d-metadata';

export type Tool = 'select' | 'node' | 'element' | 'fixed' | 'dimension' | 'support' | 'load' | 'cylinder' | 'pan' | 'influenceLine';
 ...
 */
export const EDIT_TOOLS: readonly Tool[] = ['node', 'element', 'fixed', 'dimension', 'support', 'load', 'cylinder'];
export type ILQuantity = 'Rz' | 'Ry' | 'Rx' | 'My' | 'Mz' | 'V' | 'M';
export type SupportTool = 'fixed' | 'pinned' | 'roller' | 'spring';
export type LoadTool = 'nodal' | 'distributed';
export type LoadDirection2D = 'globalX' | 'globalZ' | 'localAxial' | 'localPerpendicular' | 'angle' | 'moment';
export type SelectMode = 'nodes' | 'elements' | 'shells' | 'loads' | 'stress' | 'supports' | 'dimensions';
...
if (hasLocalStorage()) {
  for (const key of ['floating-tools', 'tooltips', 'help-panel', 'unitSystem', 'stressUnit']) {
...
function createUIStore() {
  ...
  let loadDirection = $state<LoadDirection2D>('globalZ');
  // 9.80665 kN is exactly 1000 kgf in the SI-mm/kgf display system.
  let loadValue = $state<number>(9.80665);
  let loadValueJ = $state<number>(9.80665); // kN/m at node J (for trapezoidal)
  let cylinderForce = $state<number>(10); // kN: + push, - pull
  ...
  let loadAngle = $state<number>(0); // degrees counter-clockwise from global +X
  ...
  let gridSize = $state<number>(initialUnitSystem === 'SI_MM' ? 0.1 : 1); // canonical meters
  ...
  let zoom = $state<number>(1000); // pixels per meter
   ...
   *
   * The bottom selection-filter strip is always a multi-kind filter. Its empty
   * visual state deliberately means "no restriction", matching CAD systems:
   * with no filter buttons pressed every selectable kind is accepted.
   *
   ...
   */
  let multiKindSelect = $state<boolean>(true);

  /** Explicit filters. Empty means all selectable kinds, not none. */
  let selectKinds = $state<Set<SelectMode>>(new Set());
  ...
  // driven highlight (result-query, diagrams, diagnostics, paste, select-all).
  ...
  let selectedDimensionId = $state<number | null>(null);
  ...
  let showNodeLabels = $state<boolean>(false);
  /**
  ...
  // Contextual tooltips (persisted in localStorage)
  ...
  let unitSystem = $state<UnitSystem>(initialUnitSystem);
  let stressUnit = $state<StressUnit>(initialStressUnit);
  ...
  // Analysis mode: Basic 2D, Basic 3D, or PRO.
  let analysisMode = $state<'2d' | '3d' | 'pro'>('2d');
  ...
  let snapToGrid3D_basic = $state<boolean>(false);
  let gridSize3D_basic = $state<number>(initialUnitSystem === 'SI_MM' ? 0.1 : 1);
  let gridExtent3D_basic = $state<number>(50);
  let showNodeLabels3D_basic = $state<boolean>(false);
  ...
  let snapToGrid3D_pro = $state<boolean>(false);
  let gridSize3D_pro = $state<number>(initialUnitSystem === 'SI_MM' ? 0.1 : 1);
  let gridExtent3D_pro = $state<number>(50);
  let showNodeLabels3D_pro = $state<boolean>(false);
  ...
  function applySelectMode(v: SelectMode) {
    ...
    selectMode = v;
    // Legacy single-kind callers still follow the primary mode. The normal
    // bottom filter is multi-kind and must survive table/panel mode changes.
  ...
  return {
    ...
    set currentTool(v: Tool) {
      // `pan` was a former pointer mode. Keep accepting it from old sessions,
      // but never restore or expose it now that panning is a middle-button
      // viewport gesture.
      const next = v === 'pan' ? 'select' : v;
      currentTool = next;
      if (EDIT_TOOLS.includes(next)) _onEditToolArmed?.();
    },
    ...
    get loadDirection() { return loadDirection; },
    set loadDirection(v: LoadDirection2D) { loadDirection = v; },
    ...
    get cylinderForce() { return cylinderForce; },
    set cylinderForce(v: number) { cylinderForce = v; },
    ...
    set zoom(v: number) { zoom = Math.max(10, Math.min(10_000, v)); },
    ...
    selectsKind(k: SelectMode): boolean {
      return multiKindSelect ? selectKinds.size === 0 || selectKinds.has(k) : selectMode === k;
    },

    /**
     * Add or remove a kind. Removing the last explicit filter returns to the
     * all-kinds default; an empty set is therefore useful rather than broken.
     */
    toggleSelectKind(k: SelectMode): void {
      ...
      if (next.has(k)) {
        next.delete(k);
        if (selectMode === k) selectMode = [...next][0] ?? 'elements';
      } else {
        next.add(k);
        selectMode = k;
      }
    ...
    get stressUnit() { return stressUnit; },
    set stressUnit(v: StressUnit) {
      stressUnit = v;
      setStressUnit(v);
      if (hasLocalStorage()) localStorage.setItem('stabileo-stressUnit', v);
    },

    /** Defaults that belong to a newly-created project, not to model clearing/import. */
    resetNewProjectPresentation() {
      if (unitSystem === 'SI_MM') {
        gridSize = 0.1;
        gridSize3D_basic = 0.1;
        gridSize3D_pro = 0.1;
      }
      snapToGrid = false;
      snapToGrid3D_basic = false;
      snapToGrid3D_pro = false;
    },
    ...
    // The web product is permanently Basic 2D. Keep the legacy setter shape so
    // old sessions/tests can assign it, but normalize every value at the edge.
    get analysisMode() { return '2d' as const; },
    set analysisMode(_v: '2d' | '3d' | 'pro') {
      analysisMode = '2d';
      viewportPresentation3D = 'native3d';
    },
    _setModelFlatnessProvider(_fn: () => boolean) { /* retained for engine compatibility */ },
    ...
    /** Top-level app mode is fixed for the Basic-only web product. */
    ...
    clearSelectedDimension() { selectedDimensionId = null; },
     ...
     *  result/query/diagnostic highlighting so local-axes "When selected"
     *  ignores it. */
    ...
    resetSession() {
      ...
      // unitSystem, embedMode, showTooltips, showHelpPanel, etc.
    },
    ...
    zoomToFit(nodes: Iterable<{ x: number; y: number }>, canvasWidth: number, canvasHeight: number): void {
      ...
      const newZoom = Math.min(availW / worldW, availH / worldH, 10_000);
