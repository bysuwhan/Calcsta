...
import { getFixture } from '../templates/fixture-index';
...
export interface NodalLoad {
  ...
  /** Render Fx/Fz as the single force vector originally entered by angle. */
  displayAsVector?: boolean;
...
/** A two-point hydraulic/pneumatic actuator load.
 * Positive force pushes its end nodes apart; negative force pulls them together. */
export interface CylinderLoad {
  id: number;
  nodeI: number;
  nodeJ: number;
  force: number; // kN, + compression/push, - tension/pull
  caseId?: number;
}
...
export type Load =
  ...
  | { type: 'cylinder'; data: CylinderLoad }
...
export interface StructureModel {
  blocks?: Blocks;
  /** CAD-style constraints used only while editing sketch geometry. */
    ...
    if (blockPreview) {
      ...
      if (b) {
        for (const id of Object.values(b.nodeIds)) {
          ...
          if (n) nodes.set(id, { ...n, ...localToWorld(worldToLocal(n, b), blockPreview.pose) });
        }
      }
    }
    if (nodeTransformPreview) {
      for (const [id, preview] of nodeTransformPreview) {
        if (nodes.has(id)) nodes.set(id, preview);
      }
    }
    return nodes;
  };
  function publishBlocks() {
    model.blocks = copyBlockData(model.blocks ?? emptyBlocks());
    model.nodes = new Map(model.nodes); model.elements = new Map(model.elements);
    model.supports = new Map(model.supports); model.loads = [...model.loads];
  }

  function sameSketchTarget(a: SketchConstraint, b: SketchConstraint): boolean {
    if (a.kind !== b.kind) return false;
    if (a.kind === 'dimension' && b.kind === 'dimension') {
      return a.measure === b.measure && a.references.every(r => b.references.some(s => s.kind === r.kind && s.id === r.id));
    }
    if (a.kind === 'fixedNode' && b.kind === 'fixedNode') return a.nodeId === b.nodeId;
    if ((a.kind === 'fixedElement' || a.kind === 'length') && (b.kind === 'fixedElement' || b.kind === 'length')) {
      return a.kind === b.kind && a.elementId === b.elementId;
    }
    if ((a.kind === 'angle' || a.kind === 'distance') && (b.kind === 'angle' || b.kind === 'distance')) {
      return a.kind === b.kind && ((a.elementA === b.elementA && a.elementB === b.elementB)
        || (a.elementA === b.elementB && a.elementB === b.elementA));
    }
    return false;
  }

  function editableSketchConstraint(world: SketchConstraint): { list: SketchConstraint[]; stored: SketchConstraint; commit: (list: SketchConstraint[]) => void } | null {
    if (editingBlockId === null) {
      return {
        list: model.sketchConstraints ?? [], stored: world,
        commit(list) { model.sketchConstraints = [...list]; },
      };
    }
    const b = model.blocks?.instances.find(x => x.id === editingBlockId);
    const def = b && model.blocks?.definitions.find(x => x.id === b.definitionId);
    if (!b || !def) return null;
    const rn = new Map(Object.entries(b.nodeIds).map(([local, id]) => [id, Number(local)]));
    const re = new Map(Object.entries(b.elementIds).map(([local, id]) => [id, Number(local)]));
    let stored = transformSketchConstraint(world, b, false);
    stored = remapSketchReferences(stored, id => rn.get(id)!, id => re.get(id)!);
    return {
      list: def.sketchConstraints ?? [], stored,
      commit(list) { def.sketchConstraints = [...list]; publishBlocks(); },
    };
  }

  function toggleSketchConstraint(world: SketchConstraint, _match?: (c: SketchConstraint) => boolean): boolean {
    const target = editableSketchConstraint(world); if (!target) return false;
    const index = target.list.findIndex(c => sameSketchTarget(c, target.stored));
    if (!_undoBatching) _pushUndo?.();
    const list = [...target.list];
    if (index >= 0) list.splice(index, 1); else list.push(target.stored);
    target.commit(list);
    return index < 0;
  }

  function setSketchConstraint(world: SketchConstraint, _match?: (c: SketchConstraint) => boolean) {
    const target = editableSketchConstraint(world); if (!target) return;
    const index = target.list.findIndex(c => sameSketchTarget(c, target.stored));
    if (!_undoBatching) _pushUndo?.();
    const stored = index >= 0 ? { ...target.stored, id: target.list[index].id } as SketchConstraint : target.stored;
    const list = [...target.list];
    if (index >= 0) list[index] = stored; else list.push(stored);
    target.commit(list);
  }

  function satisfySketchConstraint(_constraint: SketchConstraint, movable: Set<number>) {
    const desired = new Map<number, Node>();
    for (const id of movable) { const n = model.nodes.get(id); if (n) desired.set(id, { ...n }); }
    const solved = solveSketchMove(model.nodes, model.elements, materializedSketchConstraints(model), desired, movable);
    for (const id of movable) {
      const n = solved.get(id); if (!n) continue;
      const current = model.nodes.get(id);
      if (current && (Math.abs(current.x - n.x) > 1e-10 || Math.abs(current.y - n.y) > 1e-10)) {
        model.nodes.set(id, n);
      }
    }
    model.nodes = new Map(model.nodes);
    modelVersion++; _onMutation?.();
  }

  return {
    _setHistoryPush(fn: (kind: SnapshotKind) => void) {
      _pushUndoAppearance = () => fn('appearance');
    ...
    get model() {
      const view = activeModel();
      return new Proxy(view, { set(_target, key, value) { Reflect.set(model, key, value); return true; } });
    },
    get nodes() { return displayNodes(); },
    get elements() { return activeModel().elements; },
    get supports() { return activeModel().supports; },
    get loads() { return activeModel().loads; },
    get blocks() { return model.blocks ?? emptyBlocks(); },
    get sketchConstraints() { return materializedSketchConstraints(model); },
    inferDimension(a: SketchReference, b: SketchReference) {
      if (![a, b].every(r => r.kind === 'node' ? this.canEditNode(r.id) : this.canEditElement(r.id))) return null;
      return inferSketchDimension(a, b, model.nodes, model.elements);
    },
    addMeasuredDimension(draft: SmartDimension, position: { x: number; y: number }) {
      if (!Number.isFinite(position.x) || !Number.isFinite(position.y)) return false;
      const fresh = this.inferDimension(...draft.references);
      if (!fresh || fresh.measure !== draft.measure) return false;
      let c: SmartDimension = {
        ...fresh,
        id: nextId.sketchConstraint++,
        position: { ...position },
      };
      if (c.measure === 'angle') {
        const [a, b] = c.references;
        const ga = a.kind === 'element' ? elementGeometry(model.elements.get(a.id)!, model.nodes) : null;
        const gb = b.kind === 'element' ? elementGeometry(model.elements.get(b.id)!, model.nodes) : null;
        const sector = ga && gb ? angleDimensionSector(ga.a, ga.angle, gb.a, gb.angle, position) : null;
        if (sector) c = { ...c, value: sector.sweep };
      }
      setSketchConstraint(c);
      return true;
    },
    moveDimension(id: number, position: { x: number; y: number }) {
      if (!Number.isFinite(position.x) || !Number.isFinite(position.y)) return false;
      const c = this.sketchConstraints.find(x => x.kind === 'dimension' && x.id === id);
      if (!c || c.kind !== 'dimension') return false;
      setSketchConstraint({ ...c, position: { ...position } });
      return true;
    },
    removeDimension(id: number) {
      const dimension = this.sketchConstraints.find(c => c.kind === 'dimension' && c.id === id);
      if (dimension?.kind !== 'dimension') return false;
      const target = editableSketchConstraint(dimension);
      if (!target) return false;
      const index = target.list.findIndex(c => c.id === target.stored.id && c.kind === 'dimension');
    ...
    editDimension(id: number, value: number, readOnly = false) {
      const c = this.sketchConstraints.find(x => x.kind === 'dimension' && x.id === id);
      if (c?.kind !== 'dimension') return false;
      if (readOnly) {
        const fresh = this.inferDimension(...c.references);
        if (!fresh) return false;
        setSketchConstraint({ ...c, value: fresh.value, readOnly: true });
        return true;
      }
      return this.applyDimension({ ...c, readOnly: false }, value);
    },
    applyDimension(draft: SmartDimension, value: number) {
      if (!Number.isFinite(value) || value < 0 || (draft.measure === 'pointDistance' && value === 0)
        || (draft.measure === 'angle' && (value <= 0 || value >= Math.PI))) return false;
      const fresh = this.inferDimension(...draft.references);
      if (!fresh || fresh.measure !== draft.measure) return false;
      const c: SmartDimension = { ...fresh, id: draft.id || nextId.sketchConstraint,
        position: draft.position ? { ...draft.position } : undefined,
        readOnly: false,
        value: Math.sign(fresh.value || 1) * value };
      const idsFor = (r: SketchReference) => {
        const e = model.elements.get(r.id);
        return r.kind === 'node' ? [r.id] : e ? [e.nodeI, e.nodeJ] : [];
      };
      const constraints = materializedSketchConstraints(model).filter(old => !sameSketchTarget(old, c));
      constraints.push(c);
      // Prefer moving the second point/line; retry with both references when it is fixed.
      const preferred = c.measure === 'pointLineDistance' ? idsFor(c.references[0]) : idsFor(c.references[1]);
      for (const ids of [preferred, c.references.flatMap(idsFor)]) {
        const movable = new Set(ids.filter(id => this.canEditNode(id)));
        const solved = solveSketchMove(model.nodes, model.elements, constraints, new Map(), movable);
        if (constraints.some(x => x.kind === 'dimension' && !x.readOnly && dimensionResiduals(x, solved, model.elements).some(v => Math.abs(v) > 1e-6))) continue;
        if (constraints.some(x => {
          if (x.kind === 'fixedNode') { const n = solved.get(x.nodeId); return n && Math.hypot(n.x - x.x, n.y - x.y) > 1e-6; }
          if (x.kind === 'fixedElement') {
            const e = model.elements.get(x.elementId);
            return e && [e.nodeI, e.nodeJ].some(id => { const n = solved.get(id)!; return Math.abs(-Math.sin(x.angle) * n.x + Math.cos(x.angle) * n.y - x.offset) > 1e-6; });
          }
          return false;
        })) continue;
        setSketchConstraint(c);
        if (!draft.id) nextId.sketchConstraint++;
        for (const id of movable) { const n = solved.get(id); if (n) this.updateNode(id, n.x, n.y, n.z); }
        return true;
      }
      return false;
    },
    isNodeSketchFixed(id: number) {
      return materializedSketchConstraints(model).some(c => c.kind === 'fixedNode' && c.nodeId === id);
    },
    isElementSketchFixed(id: number) {
      return materializedSketchConstraints(model).some(c => c.kind === 'fixedElement' && c.elementId === id);
    },
    toggleSketchFixedNode(id: number) {
      if (!this.canEditNode(id)) return false;
      const world: SketchConstraint = { id: nextId.sketchConstraint++, kind: 'fixedNode', nodeId: id,
        x: model.nodes.get(id)!.x, y: model.nodes.get(id)!.y };
      return toggleSketchConstraint(world, c => c.kind === 'fixedNode' && c.nodeId === id);
    },
    toggleSketchFixedElement(id: number) {
      if (!this.canEditElement(id)) return false;
      const e = model.elements.get(id), g = e && elementGeometry(e, model.nodes);
      if (!e || !g || g.length < 1e-10) return false;
      const nx = -Math.sin(g.angle), ny = Math.cos(g.angle);
      const world: SketchConstraint = { id: nextId.sketchConstraint++, kind: 'fixedElement', elementId: id,
        angle: g.angle, offset: nx * g.a.x + ny * g.a.y };
      return toggleSketchConstraint(world, c => c.kind === 'fixedElement' && c.elementId === id);
    },
    setSketchLength(elementId: number, value?: number) {
      const e = model.elements.get(elementId), g = e && elementGeometry(e, model.nodes);
      if (!e || !g || !this.canEditElement(elementId)) return false;
      const target = value && value > 0 ? value : g.length;
      const c: SketchConstraint = { id: nextId.sketchConstraint++, kind: 'length', elementId, value: target };
      setSketchConstraint(c, x => x.kind === 'length' && x.elementId === elementId);
      satisfySketchConstraint(c, new Set([e.nodeJ]));
      return true;
    },
    setSketchAngle(elementA: number, elementB: number, valueDegrees?: number) {
      const ea = model.elements.get(elementA), eb = model.elements.get(elementB);
      const ga = ea && elementGeometry(ea, model.nodes), gb = eb && elementGeometry(eb, model.nodes);
      if (!ea || !eb || !ga || !gb || elementA === elementB || !this.canEditElement(elementA) || !this.canEditElement(elementB)) return false;
      const current = Math.atan2(Math.sin(gb.angle - ga.angle), Math.cos(gb.angle - ga.angle));
      const target = valueDegrees && valueDegrees > 0 ? Math.sign(current || 1) * valueDegrees * Math.PI / 180 : current;
      const c: SketchConstraint = { id: nextId.sketchConstraint++, kind: 'angle', elementA, elementB, value: target };
      setSketchConstraint(c, x => x.kind === 'angle' && ((x.elementA === elementA && x.elementB === elementB) || (x.elementA === elementB && x.elementB === elementA)));
      satisfySketchConstraint(c, new Set([eb.nodeJ]));
      return true;
    },
    setSketchDistance(elementA: number, elementB: number, value?: number) {
      const ea = model.elements.get(elementA), eb = model.elements.get(elementB);
      const ga = ea && elementGeometry(ea, model.nodes), gb = eb && elementGeometry(eb, model.nodes);
      if (!ea || !eb || !ga || !gb || elementA === elementB || !this.canEditElement(elementA) || !this.canEditElement(elementB)) return false;
      const parallel = Math.abs(Math.sin(gb.angle - ga.angle)) < 1e-4;
      const blockAngle = model.blocks?.instances.find(b => b.id === editingBlockId)?.angle ?? 0;
      const horizontal = Math.abs(Math.sin(ga.angle - blockAngle)) < 1e-4;
      if (!parallel || !horizontal) return false;
      const nx = -Math.sin(ga.angle), ny = Math.cos(ga.angle);
      const current = ((gb.a.x + gb.b.x - ga.a.x - ga.b.x) / 2) * nx + ((gb.a.y + gb.b.y - ga.a.y - ga.b.y) / 2) * ny;
      const target = value && value > 0 ? Math.sign(current || 1) * value : current;
      const c: SketchConstraint = { id: nextId.sketchConstraint++, kind: 'distance', elementA, elementB, axisAngle: ga.angle, value: target };
      setSketchConstraint(c, x => x.kind === 'distance' && ((x.elementA === elementA && x.elementB === elementB) || (x.elementA === elementB && x.elementB === elementA)));
      satisfySketchConstraint(c, new Set([eb.nodeI, eb.nodeJ]));
      return true;
    },
    updateNodesConstrained(desired: Map<number, Node>) {
      const movable = new Set([...desired.keys()].filter(id => this.canEditNode(id)));
      const solved = solveSketchMove(model.nodes, model.elements, materializedSketchConstraints(model), desired, movable);
      for (const id of movable) {
        const n = solved.get(id); if (n) this.updateNode(id, n.x, n.y, n.z);
      }
    },
    get editingBlockId() { return editingBlockId; },
    get blockPreview() { return blockPreview; },
    blockForNode(id: number) { return blockForNode(model.blocks ?? emptyBlocks(), id); },
    ...
    createBlock(elementIds: number[], nodeIds: number[], origin: Point2, name: string) {
      if (editingBlockId !== null) throw new Error('블록 편집을 먼저 완료하세요.');
      // Validate on an owned copy before taking the history entry.
      const draft = { ...model, nodes: new Map(model.nodes), elements: new Map(model.elements),
        supports: new Map([...model.supports].map(([id, s]) => [id, { ...s }])),
        loads: copyBlockData(model.loads), blocks: copyBlockData(model.blocks ?? emptyBlocks()),
        sketchConstraints: copyBlockData(model.sketchConstraints ?? []) };
      const ids = { ...nextId };
      const id = createBlock(draft, elementIds, nodeIds, origin, name, ids);
      _pushUndo?.(); model.blocks = draft.blocks; model.nodes = draft.nodes; model.elements = draft.elements;
      model.supports = draft.supports; model.loads = draft.loads; model.sketchConstraints = draft.sketchConstraints; nextId = ids; publishBlocks();
      return id;
    },
    duplicateBlock(id: number) {
      const b = model.blocks?.instances.find(b => b.id === id);
      if (!b || editingBlockId !== null) return null;
      _pushUndo?.();
      const clone = { ...copyBlockData(b), id: model.blocks!.nextId++, name: `${b.name} 복제`, color: nextBlockColor(model.blocks!.instances), visible: true, nodeIds: {}, elementIds: {} };
      model.blocks!.instances.push(clone); materializeBlock(model, clone, nextId); publishBlocks();
      return clone.id;
    },
    makeBlockIndependent(id: number) {
      const b = model.blocks?.instances.find(b => b.id === id);
      if (!b || editingBlockId !== null) return;
      _pushUndo?.();
      const def = copyBlockData(model.blocks!.definitions.find(d => d.id === b.definitionId)!);
      def.id = model.blocks!.nextId++; b.definitionId = def.id;
      model.blocks!.definitions.push(def); publishBlocks();
    },
    setBlockAppearance(id: number, patch: { name?: string; color?: string }) {
      const b = model.blocks?.instances.find(b => b.id === id); if (!b || editingBlockId !== null) return;
      const color = patch.color?.toLowerCase();
      if (color !== undefined && !BLOCK_COLOR_PALETTE.some(candidate => candidate === color)) return;
      _pushUndoAppearance?.(); Object.assign(b, patch, color === undefined ? {} : { color }); publishBlocks();
    },
    restoreBlockAppearance(snapshot: ModelSnapshot) {
      for (const b of model.blocks?.instances ?? []) {
        const old = snapshot.blocks?.instances.find(s => s.id === b.id);
        if (old) { b.name = old.name; b.color = old.color; }
      }
      publishBlocks();
    },
    setBlockVisible(id: number, visible: boolean) {
      const b = model.blocks?.instances.find(b => b.id === id); if (!b || editingBlockId !== null) return;
      _pushUndo?.(); b.visible = visible; publishBlocks();
    },
    deleteBlock(id: number) {
      const b = model.blocks?.instances.find(b => b.id === id); if (!b || editingBlockId !== null) return;
      _pushUndo?.();
      for (const nid of Object.values(b.nodeIds)) model.nodes.delete(nid);
      for (const eid of Object.values(b.elementIds)) model.elements.delete(eid);
      model.blocks!.instances = model.blocks!.instances.filter(b => b.id !== id);
    ...
    placeBlock(id: number, pose: BlockPose, pin?: { source: number; target: number }) {
      ...
      if (pin && Object.values(b.nodeIds).includes(pin.source) && !Object.values(b.nodeIds).includes(pin.target)) {
        ...
        if (a && target && Math.hypot(a.x - target.x, a.y - target.y) < 1e-7) {
          model.blocks!.joints.push({ id: model.blocks!.nextId++, a: nodeRef(model.blocks!, pin.source), b: nodeRef(model.blocks!, pin.target), kind: 'pin' });
        }
      }
      publishBlocks();
    },
    removeBlockJoint(id: number) {
      ...
      _pushUndo?.(); model.blocks!.joints = model.blocks!.joints.filter(j => j.id !== id); publishBlocks();
    },
    beginBlockEdit(id: number) {
      const b = model.blocks?.instances.find(b => b.id === id);
      if (!b?.visible || editingBlockId !== null) return;
      blockEditSnapshot = this.snapshot(); editingBlockId = id; _undoBatching = true;
    },
    finishBlockEdit(commit: boolean) {
      if (editingBlockId === null || !blockEditSnapshot) return null;
      const b = model.blocks!.instances.find(b => b.id === editingBlockId)!;
      let removed = { joints: 0, supports: 0, loads: 0 };
      if (commit) {
        const oldNodes = new Set(blockEditSnapshot.nodes.map(([id]) => id));
        const oldElements = new Set(blockEditSnapshot.elements.map(([id]) => id));
        for (const id of model.nodes.keys()) if (!oldNodes.has(id) && !blockForNode(model.blocks!, id)) b.nodeIds[id] = id;
        for (const id of model.elements.keys()) if (!oldElements.has(id) && !blockForElement(model.blocks!, id)) b.elementIds[id] = id;
        const def = model.blocks!.definitions.find(d => d.id === b.definitionId)!;
        const reverseNodes = new Map(Object.entries(b.nodeIds).map(([local, id]) => [id, Number(local)]));
        const reverseElements = new Map(Object.entries(b.elementIds).map(([local, id]) => [id, Number(local)]));
        def.nodes = [...reverseNodes].flatMap(([id, local]) => {
          const n = model.nodes.get(id); return n ? [{ ...n, id: local, ...worldToLocal(n, b) }] : [];
        });
        def.elements = [...reverseElements].flatMap(([id, local]) => {
          const e = model.elements.get(id); return e ? [{ ...copyBlockData(e), id: local, nodeI: reverseNodes.get(e.nodeI)!, nodeJ: reverseNodes.get(e.nodeJ)! }] : [];
        });
        const localNodes = new Set(def.nodes.map(n => n.id));
        const localElements = new Set(def.elements.map(e => e.id));
        def.sketchConstraints = (def.sketchConstraints ?? []).filter(c => {
          return sketchReferences(c).every(r => r.kind === 'node' ? localNodes.has(r.id) : localElements.has(r.id));
        });
        for (const instance of model.blocks!.instances.filter(i => i.definitionId === def.id)) materializeBlock(model, instance, nextId);
        removed = cleanBlockReferences(model);
        removed.supports = blockEditSnapshot.supports.length - model.supports.size;
        removed.loads = blockEditSnapshot.loads.length - model.loads.length;
        publishBlocks();
      }
      const after = commit ? this.snapshot(true) : null;
      const before = blockEditSnapshot;
      blockEditSnapshot = null; editingBlockId = null; _undoBatching = false;
      this.restore(before);
      if (after) { _pushUndo?.(); this.restore(after); }
      return removed;
    },
    ...
    get constraints() { return activeModel().constraints; },
    get connectors() { return activeModel().connectors; },
    ...
    snapshot(includeDraft = false): ModelSnapshot {
      if (!includeDraft && blockEditSnapshot) return plainDeepCopy(blockEditSnapshot);
      ...
      const result: ModelSnapshot = {
        blocks: copyBlockData(snap.blocks ?? emptyBlocks()),
        sketchConstraints: copyBlockData(snap.sketchConstraints ?? []),
        name: snap.name,
    ...
    restore(rawSnapshot: ModelSnapshot): void {
      ...
      model.blocks = s.blocks ?? emptyBlocks();
      model.sketchConstraints = copyBlockData(s.sketchConstraints ?? []);
      editingBlockId = null; blockEditSnapshot = null; blockPreview = null; nodeTransformPreview = null; _undoBatching = false;
      modelVersion++;
      _onMutation?.();
      ...
      const supEntries = s.supports.map(([k, v]) => [k, {
        ...
        dz: canonicalSupportDz(v as Partial<Support>),
        dry: canonicalSupportDry(v as Partial<Support>),
      }] as [number, Support]);
      ...
      model.loads = (s.loads as unknown as Array<{ type: string; data: Record<string, unknown> }>)
      ...
      model.loadCases = s.loadCases
        ...
        : [{ id: 1, type: 'A' as LoadCaseType, name: 'A' }];
      ...
      nextId.loadCase = s.nextId.loadCase ?? Math.max(0, ...model.loadCases.map(c => c.id)) + 1;
      nextId.combination = s.nextId.combination ?? Math.max(0, ...model.combinations.map(c => c.id)) + 1;
      nextId.plate = s.nextId.plate ?? 1;
      nextId.quad = s.nextId.quad ?? 1;
      nextId.connector = (s.nextId as any).connector ?? 1;
      nextId.sketchConstraint = s.nextId.sketchConstraint ?? (Math.max(0,
        ...(s.sketchConstraints ?? []).map(c => c.id),
        ...(s.blocks?.definitions ?? []).flatMap(d => (d.sketchConstraints ?? []).map(c => c.id)),
      ) + 1);
    ...
    addNode(x: number, y: number, z?: number, opts?: { autoCreated?: boolean }): number {
      if (!_undoBatching) _pushUndo?.();
      const id = nextId.node++;
      const node: Node = { id, x, y };
      if (z !== undefined && z !== 0) node.z = z;
      if (opts?.autoCreated) node.autoCreated = true;
      model.nodes.set(id, node);
      if (editingBlockId !== null) model.blocks!.instances.find(b => b.id === editingBlockId)!.nodeIds[id] = id;
      if (!_bulkMutating) model.nodes = new Map(model.nodes);
      return id;
    },

    updateNodeZ(id: number, z: number): void {
      if (!this.canEditNode(id)) return;
    ...
    addElement(nodeI: number, nodeJ: number, type: 'frame' | 'truss' = 'frame', opts?: { hingedEnds?: boolean }): number {
      if (nodeI === nodeJ) return -1;
      if (editingBlockId !== null) {
        if (!this.canEditNode(nodeI) || !this.canEditNode(nodeJ)) return -1;
      } else {
        if (!model.nodes.has(nodeI) || !model.nodes.has(nodeJ)) return -1;
      }
      if (!_undoBatching) _pushUndo?.();
      ...
      model.elements.set(id, {
        ...
        materialId: 1,
        sectionId: 1,
        releaseI: { ...NO_RELEASE, mz: opts?.hingedEnds === true },
        releaseJ: { ...NO_RELEASE, mz: opts?.hingedEnds === true },
      });
      if (!_bulkMutating) model.elements = new Map(model.elements);
      if (editingBlockId !== null) model.blocks!.instances.find(b => b.id === editingBlockId)!.elementIds[id] = id;
      return id;
    },
    ...
    addSupport(nodeId: number, type: SupportType, springs?: { kx?: number; ky?: number; kz?: number; krx?: number; kry?: number; krz?: number }, opts?: { angle?: number; isGlobal?: boolean; dx?: number; dy?: number; dz?: number; drx?: number; dry?: number; drz?: number; dofRestraints?: { tx: boolean; ty: boolean; tz: boolean; rx: boolean; ry: boolean; rz: boolean }; dofFrame?: 'global' | 'local'; dofLocalElementId?: number }): number {
      if (editingBlockId !== null) return -1;
      if (!_undoBatching) _pushUndo?.();
    ...
    addNodalLoad(nodeId: number, fx: number, fz: number, my: number = 0, caseId?: number, displayAsVector = false): number {
      if (editingBlockId !== null) return -1;
      if (!_undoBatching) _pushUndo?.();
      const id = nextId.load++;
      const data: NodalLoad = { id, nodeId, fx, fz, my };
      if (caseId !== undefined) data.caseId = caseId;
      if (displayAsVector) data.displayAsVector = true;
    ...
    addCylinderLoad(nodeI: number, nodeJ: number, force: number, caseId?: number): number {
      if (editingBlockId !== null || nodeI === nodeJ || !model.nodes.has(nodeI) || !model.nodes.has(nodeJ)) return -1;
      if (!_undoBatching) _pushUndo?.();
      const id = nextId.load++;
      const data: CylinderLoad = { id, nodeI, nodeJ, force };
      if (caseId !== undefined) data.caseId = caseId;
      const entry = { type: 'cylinder' as const, data };
      if (_bulkLoadBuffer) _bulkLoadBuffer.push(entry);
      else model.loads = [...model.loads, entry];
      return id;
    },
    ...
    removeNode(id: number): void {
      ...
      const keepLoad = (l: Load) =>
        ...
        && !(l.type === 'cylinder' && (l.data.nodeI === id || l.data.nodeJ === id));
      ...
      model.sketchConstraints = (model.sketchConstraints ?? []).filter(c => {
        return sketchReferences(c).every(r => r.kind === 'node' ? model.nodes.has(r.id) : model.elements.has(r.id));
      });
      ...
      if (editingBlockId === null && model.blocks) { cleanBlockReferences(model); publishBlocks(); }
    },

    isNodeOrphaned(id: number): boolean {
      if (!model.nodes.has(id)) return false;
      for (const elem of model.elements.values()) {
        if (elem.nodeI === id || elem.nodeJ === id) return false;
      }
      for (const plate of model.plates.values()) {
        if (plate.nodes.includes(id)) return false;
      }
      for (const quad of model.quads.values()) {
        if (quad.nodes.includes(id)) return false;
      }
      for (const sup of model.supports.values()) {
        if (sup.nodeId === id) return false;
      }
      for (const l of model.loads) {
        if ((l.type === 'nodal' || l.type === 'nodal3d') && (l.data as any).nodeId === id) return false;
        if (l.type === 'cylinder' && (l.data.nodeI === id || l.data.nodeJ === id)) return false;
      }
      for (const conn of model.connectors.values()) {
        if (conn.nodeI === id || conn.nodeJ === id) return false;
      }
      for (const f of model.footings.values()) {
        if (f.nodeId === id) return false;
      }
      if ((model.sketchConstraints ?? []).some(c => c.kind === 'fixedNode' && c.nodeId === id)) return false;
      if (model.blocks) {
        for (const b of model.blocks.instances) {
          if (b.nodeIds && b.nodeIds[id] !== undefined) return false;
        }
      }
      for (const c of model.constraints) {
        if (c.type === 'diaphragm') {
          if (c.masterNode === id || c.slaveNodes.includes(id)) return false;
        } else if (c.type === 'linearMPC') {
          if (c.terms.some(t => t.nodeId === id)) return false;
        } else {
          if (c.masterNode === id || c.slaveNode === id) return false;
        }
      }
      return true;
    },
    ...
    removeElement(id: number): void {
      if (!this.canEditElement(id)) return;
      const elem = model.elements.get(id);
      if (!elem) return;
      const { nodeI, nodeJ } = elem;

      const run = () => {
        model.elements.delete(id);
        model.elements = new Map(model.elements);
        model.sketchConstraints = (model.sketchConstraints ?? []).filter(c => !sketchReferences(c).some(r => r.kind === 'element' && r.id === id));
        ...
        // Clean up orphaned end nodes (like AutoCAD line endpoints when erased)
        for (const nid of [nodeI, nodeJ]) {
          if (model.nodes.get(nid)?.autoCreated && this.isNodeOrphaned(nid)) {
            this.removeNode(nid);
          }
        }
      };
    ...
    updateLoad(loadId: number, data: Record<string, number | boolean | undefined>): void {
      ...
      if (load.type === 'nodal') {
        ...
        if (data.force !== undefined) load.data.force = data.force as number;
    ...
    clear(): void {
      model.blocks = emptyBlocks(); editingBlockId = null; blockEditSnapshot = null; blockPreview = null; nodeTransformPreview = null; _undoBatching = false;
      if (!_undoBatching) _pushUndo?.();
      ...
      nextId.load = 1;
      nextId.loadCase = 2;
      nextId.combination = 2;
      nextId.plate = 1;
      nextId.quad = 1;
      nextId.connector = 1;
      nextId.footing = 1;
      nextId.soilProfile = 1;
      nextId.sketchConstraint = 1;
      model.provenance = undefined;
      lastKinematicResult = null;
      // Basic always uses the native 2D presentation.
    },

    /** Replace model geometry data in-place. No undo. */
    ...
    subdivideElement(elementId: number, n: number): void {
      if (!this.canEditElement(elementId)) return;
      if (n < 2 || n > 20) return;
      ...
      model.nodes = new Map(model.nodes);
      model.elements = new Map(model.elements);
      _undoBatching = editingBlockId !== null;
    },
    ...
    /** Set every frame end meeting at a node to one explicit connection kind. */
    setNodeHinge(nodeId: number, hinged: boolean): void {
      if (!this.canEditNode(nodeId)) return;
      const connected = [...model.elements.values()].filter(element =>
        element.type === 'frame' && (element.nodeI === nodeId || element.nodeJ === nodeId));
      if (!connected.some(element => {
        const release = element.nodeI === nodeId ? element.releaseI : element.releaseJ;
        return (release?.mz === true) !== hinged;
      })) return;
      if (!_undoBatching) _pushUndo?.();
      for (const element of connected) {
        const plain = $state.snapshot(element) as Element;
        if (plain.nodeI === nodeId) plain.releaseI = { ...(plain.releaseI ?? NO_RELEASE), mz: hinged };
        if (plain.nodeJ === nodeId) plain.releaseJ = { ...(plain.releaseJ ?? NO_RELEASE), mz: hinged };
        model.elements.set(plain.id, plain);
      }
      if (!_bulkMutating) model.elements = new Map(model.elements);
    },
     ...
     *  Redistributes distributed and point loads to the sub-elements.
    ...
    splitElementAtPoint(elementId: number, t: number): { nodeId: number; elemA: number; elemB: number } | null {
      if (!this.canEditElement(elementId)) return null;
      ...
      model.loads = model.loads.filter(l => {
        if (l.type === 'distributed' || l.type === 'pointOnElement') {
    ...
    mirrorNodes(nodeIds: Set<number>, axis: 'x' | 'y'): void {
      nodeIds = new Set([...nodeIds].filter(id => this.canEditNode(id)));
    ...
    solve(includeSelfWeight = false, drawPlane: DrawPlane = 'xy'): AnalysisResults | string | null {
      if (editingBlockId !== null) return '블록 편집을 완료한 뒤 해석하세요.';
    ...
    computeInfluenceLine(
      ...
      if (activeModel().constraints.length) return '핀 또는 연결 구속이 있는 모델의 영향선 해석은 지원하지 않습니다.';
      return computeInfluenceLineFn(
        { nodes: activeModel().nodes, elements: activeModel().elements, supports: activeModel().supports,
          loads: activeModel().loads, materials: model.materials, sections: model.sections },
    ...
    updateElementMaterial(elemId: number, materialId: number): void {
      if (!this.canEditElement(elemId)) return;
      if (!_undoBatching) _pushUndo?.();
      const elem = model.elements.get(elemId);
      if (!elem) return;
      const plain = $state.snapshot(elem) as Element;
