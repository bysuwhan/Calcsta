...
export interface ModelSnapshot {
  blocks?: import('../model/blocks').Blocks;
  /** Sketch-only geometric constraints; never structural support conditions. */
  sketchConstraints?: import('../model/sketch-constraints').SketchConstraint[];
  name?: string;
  /** Legacy envelope metadata accepted on read, never emitted by Basic 2D saves. */
  appMode?: 'basico' | string;
  ...
  nodes: Array<[number, { id: number; x: number; y: number; z?: number; autoCreated?: boolean }]>;
  ...
  nextId: { node: number; material: number; section: number; element: number; support: number; load: number; loadCase?: number; combination?: number; plate?: number; quad?: number; connector?: number; footing?: number; soilProfile?: number; sketchConstraint?: number };
 ...
 *    survive untouched.
 */
export type SnapshotKind = 'structural' | 'reinforcement' | 'foundation' | 'appearance';
...
function createHistoryStore() {
  ...
  const store = {
    ...
    undo(): void {
      ...
      redoKinds.push(kind);
      if (kind === 'appearance') {
        modelStore.restoreBlockAppearance(prev);
    ...
    redo(): void {
      ...
      undoKinds.push(kind);
      if (kind === 'appearance') {
        modelStore.restoreBlockAppearance(next);
      } else if (kind === 'reinforcement') {
