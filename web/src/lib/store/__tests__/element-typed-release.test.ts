...
describe('Element typed Release contract', () => {
  ...
  it('can create the hinged frame used by the interactive line tool', () => {
    modelStore.addNode(0, 0);
    modelStore.addNode(6, 0);
    const elemId = modelStore.addElement(1, 2, 'frame', { hingedEnds: true });
    expect(modelStore.elements.get(elemId)!.releaseI.mz).toBe(true);
    expect(modelStore.elements.get(elemId)!.releaseJ.mz).toBe(true);
  });

  it('sets every frame end at a point to hinge or rigid in one operation', () => {
    modelStore.addNode(0, 0);
    modelStore.addNode(-2, 0);
    modelStore.addNode(2, 0);
    const left = modelStore.addElement(2, 1, 'frame');
    const right = modelStore.addElement(1, 3, 'frame', { hingedEnds: true });

    modelStore.setNodeHinge(1, true);
    expect(modelStore.elements.get(left)!.releaseJ.mz).toBe(true);
    expect(modelStore.elements.get(right)!.releaseI.mz).toBe(true);

    modelStore.setNodeHinge(1, false);
    expect(modelStore.elements.get(left)!.releaseJ.mz).toBe(false);
    expect(modelStore.elements.get(right)!.releaseI.mz).toBe(false);
  });
