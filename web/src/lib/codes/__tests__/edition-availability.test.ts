...
describe('selectors expose only AVAILABLE editions', () => {
  ...
  it('CIRSOC 201-2005 specifically: reserved, explained, not selectable', () => {
    ...
    expect(findRegulation('cirsoc-201', '2005')?.sourceReviewed).toBe(false);
    expect(findRegulation('cirsoc-201', '2025')?.sourceReviewed).toBe(true);
  });
});
...
describe('every implemented rule cites the edition it came from', () => {
  ...
  it('the registry never marks an edition available without source review', () => {
    // Availability is downstream of maintainer review against the official edition.
    // The official publication itself is not redistributed with Calcsta.
    for (const role of REGULATION_ROLES) {
      for (const o of optionsForRole(role)) {
        if (!o.regulation) continue;
        const info = findRegulation(o.regulation, o.edition as never);
        if (!info) continue;
        if (!info.sourceReviewed) {
          // An available option whose text is absent is exactly the state this test forbids
          // for the design-governing concrete role.
          expect(
            o.role === 'concrete',
            `${o.adapterId} is AVAILABLE for role ${o.role} without source review`,
          ).toBe(false);
        }
      }
    }
