...
const PUBLIC_AGPL = path.join(WEB, 'public', 'LICENSE.txt');
...
describe('third-party notices', () => {
  ...
  it('are reproduced verbatim in what the site serves', () => {
    ...
    expect(fs.readFileSync(PUBLIC_AGPL, 'utf8')).toBe(
      fs.readFileSync(path.join(REPO, 'LICENSE'), 'utf8'),
    );
  });
  ...
  it('carry every licence family that reaches the Basic bundle', () => {
    const text = fs.readFileSync(NOTICES, 'utf8');
    // Named individually rather than counted so a dependency cannot silently
    // disappear from the generated notice.
    expect(text, 'MIT packages').toContain('| MIT |');
    expect(text, 'Apache-2.0 packages').toContain('Apache-2.0');
    expect(text).toContain('| Cargo | `wasm-bindgen` |');
    expect(text).toContain('| data | `eurocodepy profile data` |');
    expect(text).not.toContain('web-ifc');
    expect(text).not.toContain('three-mesh-bvh');
  ...
  it('reproduce real copyright lines, not a summary of them', () => {
    ...
    expect(text).toContain('Copyright (c) 2026 Paulo Cachim');  // eurocodepy profile data
  });
  ...
  it('do not list build tooling, which is not distributed', () => {
    ...
    for (const dev of ['`vite` |', '`vitest` |', '`svelte` |', '`typescript` |', '`criterion` |']) {
