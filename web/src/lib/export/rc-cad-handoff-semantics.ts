 ...
 *   * a containment or cover PASS while Calcsta's own status is NOT_EVALUATED — the exact
...
export function validateRcCadHandoffSemantics(doc: unknown): SemanticViolation[] {
  ...
  // ── 7. No verdict Calcsta did not reach ────────────────────
  for (const c of checks) {
    if (c.evaluationStatus === 'NOT_EVALUATED') {
      ...
      if (c.authority === 'stabileo') {
        fail('check.authorityWithoutEvaluation',
          `check "${c.checkId}" claims Calcsta authority but was not evaluated`);
      }
  ...
  // containment check that claims a Calcsta verdict is wrong by construction.
  for (const c of checks) {
    ...
    if (c.evaluationStatus === 'EVALUATED' || c.authority === 'stabileo') {
      fail('containment.notEvaluatedInThisRelease',
        `check "${c.checkId}" reports a containment verdict; Calcsta has no production `
