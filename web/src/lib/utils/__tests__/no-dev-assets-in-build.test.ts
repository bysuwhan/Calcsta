 ...
 * to everybody, because a built site was once driven in a headless browser served
 * from localhost: the swap fired, and the captured `document.head` happened after
 * it. The deployed page declared `/favicon-dev.svg` until somebody noticed by eye.
 *
 ...
 *
 * If the dev swap is absent from the emitted JavaScript, no browser driving the
 * built app can produce a page that names the dev icon. Reading a checked-out
 * `dist/` would prove less and cost more: it is
 ...
 * `http://localhost:3001` — the local development backend — so
...
describe('the build carries nothing that is local-only', () => {
  ...
  it(
    ...
    async () => {
      ...
      expect(link).toMatch(/href="\/logo\.svg"/);
    },
    300_000,
  );
  ...
  it('the dev swap is gated on the build-time flag, so the bundler can drop it', () => {
    ...
    // browser driving the built app from a local HTTP server.
