/** Small structural vector used by the 2D-facing geometry helpers.  Keeping
 * this module dependency-free prevents the removed WebGL/Three.js product
 * stack from being pulled into the Basic bundle. */
...
function vector(x: number, y: number, z: number): SceneVector {
  return Object.freeze({ x, y, z, clone: () => vector(x, y, z), toArray: () => [x, y, z] as [number, number, number] });
}
 ...
 */
export const GLOBAL_X: Readonly<SceneVector> = vector(1, 0, 0);
export const GLOBAL_Y: Readonly<SceneVector> = vector(0, 1, 0);
export const GLOBAL_Z: Readonly<SceneVector> = vector(0, 0, 1);

export const UP_VECTOR: Readonly<SceneVector> = GLOBAL_Z;
export const GRAVITY_VECTOR_3D: Readonly<SceneVector> = vector(0, 0, -1);
export const TOP_VIEW_UP_VECTOR: Readonly<SceneVector> = GLOBAL_Y;
 ...
 */
export const THREEJS_CYLINDER_AXIS: Readonly<SceneVector> = GLOBAL_Y;
...
export function setCameraUp(camera: { up: { copy(v: SceneVector): unknown } }): void {
...
export function planeNormal(plane: WorkingPlane3D): SceneVector {
  switch (plane) {
...
export function setPlaneOffset(target: { position: { x: number; y: number; z: number; set(x: number, y: number, z: number): unknown }; rotation: { set(x: number, y: number, z: number): unknown; x: number; z: number } }, plane: WorkingPlane3D, level: number): void {
...
export function toSceneVector(point: ScenePoint): SceneVector {
