...
export async function initSolver(): Promise<void> {
  ...
  wasmInitPromise = (async () => {
    ...
    // The generated wasm-bindgen module is replaced during the build. Keep its
    // surface opaque here so the web app does not need to duplicate the full
    // (including engine-only 3D) export list in a handwritten declaration.
    const wasm: any = await import('../wasm/dedaliano_engine.js');
...
export function solveModal(
  ...
  const payload = JSON.stringify({
    solver: {
      ...
      loads: input.loads,
      constraints: plainDeepCopy(input.constraints ?? []),
      connectors: mapToObj(input.connectors ?? new Map()),
    },
...
export function solveSpectral(config: {
  ...
  const payload = JSON.stringify({
    solver: {
      ...
      constraints: plainDeepCopy(config.solver.constraints ?? []),
      connectors: mapToObj(config.solver.connectors ?? new Map()),
    },
    modes: config.modes,
...
export function solvePlastic(config: {
  ...
  if (config.solver.constraints?.length) throw new Error('핀 또는 연결 구속이 있는 모델의 소성 해석은 지원하지 않습니다.');
