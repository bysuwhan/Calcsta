...
function detail() {
  ...
  return runDetailing({
    ...
    reverify: (elementId, depthLoss) => {
      ...
      if (!ctx) return 'fail';
      const d = ctx.section.h - (ctx.material.cover ?? 0) - ctx.material.stirrupDia / 1000;
      const loss = Math.max(depthLoss.bottomRaise, depthLoss.topLower) + depthLoss.depthTolerance;
      return d - loss > 0.5 * ctx.section.h ? 'ok' : 'warn';
    },
  });
}
