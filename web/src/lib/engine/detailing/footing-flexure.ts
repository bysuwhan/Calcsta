...
export interface FootingMatRegion {
  ...
  asRequired: number;
  /**
   * `0,0018 A_g` evaluated on THIS region's strip, m² — a Calcsta policy figure, not a
...
export interface FootingDirectionDesign {
   ...
   * the Calcsta regional-minimum policy is code-compliant, and filing that next to a real
...
function designDirection(
  ...
  const addRegion = (
     ...
     * per region is a Calcsta conservative preference, and presenting it as the code's
