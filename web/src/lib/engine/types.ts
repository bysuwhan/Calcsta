...
/**
 * 2D wire node. `z` is canonical; `y` remains accepted for persisted and
 * legacy solver fixtures (Rust deserializes it through `alias = "y"`).
 */
export type SolverNode = ({ id: number; x: number; z: number } | { id: number; x: number; y: number })
...
export type SupportType = 'fixed' | 'pinned' | 'rollerX' | 'rollerY' | 'rollerZ' | 'spring' | 'inclinedRoller';
...
export interface SolverSupport {
  ...
  /** Legacy aliases accepted by the Rust input schema. */
  dy?: number;
  drz?: number;
...
/** Canonical x-z load components plus the legacy fy/mz wire aliases. */
export type SolverNodalLoad = (
  { nodeId: number; fx: number; fz: number; my: number; fy?: never; mz?: never }
  | { nodeId: number; fx: number; fy: number; mz: number; fz?: never; my?: never }
);
...
export type Displacement = (
  { nodeId: number; ux: number; uz: number; ry: number }
  | { nodeId: number; ux: number; uy: number; rz: number }
) & Record<string, number>;

export type Reaction = (
  { nodeId: number; rx: number; rz: number; my: number }
  | { nodeId: number; rx: number; ry: number; mz: number }
) & Record<string, number>;
