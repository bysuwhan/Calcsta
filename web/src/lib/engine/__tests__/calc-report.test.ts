...
describe('calc-report reactions section', () => {
  ...
  it('renders SI_MM values without mutating canonical results', () => {
    const data = baseData({ unitSystem: 'SI_MM', nodes: [{ id: 1, x: 1.000001, y: 0 }] as any });
    const before = data.results2D!.reactions[0].rz;
    const html = generateCalcReportHtml(data);
    expect(html).toContain('X (mm)');
    expect(html).toContain('1000.001');
    expect(html).toContain('Rx (kgf)');
    expect(data.results2D!.reactions[0].rz).toBe(before);
  });
