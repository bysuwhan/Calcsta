...
import type {
  ...
  DistributedLoad, PointLoadOnElement, CylinderLoad,
...
export interface ModelData {
  ...
  quads?: Map<number, { id: number; nodes: [number, number, number, number]; materialId: number; thickness: number; curved?: boolean }>;
...
function cylinderNodalLoads2D(model: ModelData, d: CylinderLoad): SolverInput['loads'] {
  const ni = model.nodes.get(d.nodeI), nj = model.nodes.get(d.nodeJ);
  if (!ni || !nj) return [];
  const dx = nj.x - ni.x, dz = nj.y - ni.y, length = Math.hypot(dx, dz);
  if (length < 1e-10) return [];
  const fx = d.force * dx / length, fz = d.force * dz / length;
  // Positive actuator force is compression: equal/opposite forces push outward.
  return [
    { type: 'nodal', data: { nodeId: d.nodeI, fx: -fx, fz: -fz, my: 0 } },
    { type: 'nodal', data: { nodeId: d.nodeJ, fx, fz, my: 0 } },
  ];
}
...
function buildSolverLoads2D(model: ModelData, loads: Load[], includeSelfWeight: boolean): SolverInput['loads'] {
  ...
  for (const l of loads) {
    if (l.type === 'nodal') {
      ...
      solverLoads.push(...cylinderNodalLoads2D(model, l.data));
      ...
      solverLoads.push({ type: 'distributed' as const, data: sd });
      ...
      solverLoads.push({ type: 'pointOnElement' as const, data: spd });
    }
  }
...
function prepareSolve2D(
  ...
  for (const l of model.loads) {
    if (l.type === 'nodal') {
      ...
      if (!model.nodes.has(l.data.nodeI) || !model.nodes.has(l.data.nodeJ)) {
        return t('svc.loadRefNodeMissing').replace('{n}', `${l.data.nodeI}/${l.data.nodeJ}`);
      }
    } else if (l.type === 'pointOnElement') {
...
function buildSolverLoads3D(model: ModelData, loads: Load[], includeSelfWeight: boolean, leftHand: boolean): SolverLoad3D[] {
  ...
  for (const l of loads) {
    if (l.type === 'nodal') {
      ...
      const fx = d.force * dx / length, fy = d.force * dy / length, fz = d.force * dz / length;
      solverLoads.push(
        { type: 'nodal', data: { nodeId: d.nodeI, fx: -fx, fy: -fy, fz: -fz, mx: 0, my: 0, mz: 0 } },
        { type: 'nodal', data: { nodeId: d.nodeJ, fx, fy, fz, mx: 0, my: 0, mz: 0 } },
      );
