 ...
 *
 * The demos load an example, press solve, wait for a result and point at a
 * ribbon command. Written out repeatedly, those four things drift, so they
 * are written once for the Basic 2D editor.
 *
 * # Anchors
 *
...
/** Whether the planar model on screen has been solved. */
export function hasResults(): boolean {
  return resultsStore.results !== null;
}

/** Keep demos explicit about the one supported analysis mode. */
export function setDimension(_d: '2d' = '2d'): void {
...
export function armTool(tool: 'node' | 'element' | 'support' | 'load' | 'select'): void {
