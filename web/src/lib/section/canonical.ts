...
export function resolveCanonicalSection(sec: Section): ResolvedSection {
  ...
  /*
   * A dimension of zero is normally missing, not a usable dimension.
   *
   ...
   * undrawn. The deliberate exception is `CHS` with `t: 0`, which is the
   * established representation of a solid circular bar and is built as a
   * circle rather than sent to the hollow-section builder.
   */
  ...
  switch (sec.shape) {
    ...
    case 'CHS': {
      const missing = need('h');
      if (sec.t == null || !Number.isFinite(sec.t) || sec.t < 0) missing.push('t');
      return missing.length
        ? propertiesOnly(sec, { kind: 'missingDimensions', missing })
        : backed(sec.t === 0
          ? buildSectionGeometry({ kind: 'circle', d: sec.h! })
          : buildSectionGeometry({ kind: 'chs', d: sec.h!, t: sec.t! }));
    }
    default:
