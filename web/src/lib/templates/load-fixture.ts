 ...
 * Both the app and tests consume the same planar JSON files, eliminating
 * mock divergence.
 */
...
export interface JSONModel {
  ...
  /** Legacy spatial arrays are retained in the reader type so malformed
   * historical fixtures can be rejected by validation, but are never loaded. */
 ...
 * materials → sections → nodes → elements → supports → planar loads
 */
export function loadFixture(json: JSONModel, api: FixtureLoader): void {
  ...
  for (const load of json.loads) {
    ...
    switch (load.type) {
      ...
      // Spatial loads are intentionally not part of the Basic fixture format.
    }
  }
