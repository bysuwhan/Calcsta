 ...
 * what let a Korean user read English trivia in a dialog nobody had checked,
 ...
 * PRO, the DXF importer and the landing page have their own gaps and
...
const ko = ALL_DICTS.ko;

const SRC = join(import.meta.dirname, '../../..');

/** Areas with their own translation debt and their own PRs. */
const OUT_OF_SCOPE = /^(pro\.|cad\.|landing\.)/;
const SKIP_DIRS = ['__tests__', 'locales', 'pro'];
...
describe('Basic mode is fully translated', () => {
  ...
  it('every key is defined in Spanish', () => {
    const missing = [...used].filter((k) => !(k in ko)).sort();
    expect(missing).toEqual([]);
  });
  ...
  it('placeholders survive translation in both target languages', () => {
    ...
    for (const k of used) {
      ...
      for (const [lang, dict] of [['ko', ko]] as const) {
        const v = dict[k];
...
describe('namespaces filled from data, which the scan cannot see', () => {
  ...
  for (const ns of NAMESPACES) {
    it(`${ns}* is complete in ko and pt`, () => {
      ...
      for (const lang of ['ko']) {
        const have = new Set(Object.keys(lang === 'ko' ? ko : en));
