...
export function drawDeformed(
  ...
  for (const d of results.displacements) {
    dispMap.set(d.nodeId, { ux: d.ux, uz: d.uz ?? d.uy, ry: d.ry ?? d.rz });
  }
...
/** Draw the deformed result of a two-node actuator as a straight line.
 * A cylinder is a load condition rather than a beam, so it has no Hermite
 * member curve or internal-force interpolation; its result geometry is simply
 * the line between its two displaced attachment nodes. */
export function drawDeformedCylinder(
  results: AnalysisResults,
  cylinder: { nodeI: number; nodeJ: number },
  dc: Pick<DrawContext, 'ctx' | 'worldToScreen' | 'getNode'>,
  scale: number,
): void {
  const ni = dc.getNode(cylinder.nodeI), nj = dc.getNode(cylinder.nodeJ);
  const di = results.displacements.find(d => d.nodeId === cylinder.nodeI);
  const dj = results.displacements.find(d => d.nodeId === cylinder.nodeJ);
  if (!ni || !nj || !di || !dj) return;
  const a = dc.worldToScreen(ni.x + di.ux * scale, ni.y + (di.uz ?? di.uy) * scale);
  const b = dc.worldToScreen(nj.x + dj.ux * scale, nj.y + (dj.uz ?? dj.uy) * scale);
  dc.ctx.save();
  dc.ctx.strokeStyle = canvasTheme().deformed;
  dc.ctx.lineWidth = 2.5;
  dc.ctx.setLineDash([6, 4]);
  dc.ctx.beginPath();
  dc.ctx.moveTo(a.x, a.y);
  dc.ctx.lineTo(b.x, b.y);
  dc.ctx.stroke();
  dc.ctx.restore();
}
