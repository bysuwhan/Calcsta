...
import { displayedDiagramScale, drawDiagrams, type DiagramKind } from '../draw-diagrams';
...
describe('display-unit diagram scale', () => {
  it('reduces only the axial diagram to one tenth in SI_MM', () => {
    expect(displayedDiagramScale('axial', 'SI_MM', 2)).toBe(0.2);
    expect(displayedDiagramScale('shear', 'SI_MM', 2)).toBe(2);
    expect(displayedDiagramScale('moment', 'SI_MM', 2)).toBe(2);
    expect(displayedDiagramScale('axial', 'SI', 2)).toBe(2);
    expect(displayedDiagramScale('axial', 'Imperial', 2)).toBe(2);
  });
});
