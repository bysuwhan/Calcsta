...
function loadDirectionLabel(angle: number, isGlobal: boolean): string {
  if (isGlobal) {
    if (Math.abs(angle) < 1e-10) return ' [Z]';
    if (Math.abs(angle - 90) < 1e-10) return ' [X]';
    return ` [X ${90 - angle}°]`;
  }
  if (Math.abs(angle - 90) < 1e-10) return ' [축]';
  return Math.abs(angle) < 1e-10 ? ' [⊥]' : ` [⊥ ${angle}°]`;
}
...
export function drawDistributedLoads(
  ...
  for (const load of loads) {
    ...
    const coordLabel = loadDirectionLabel(loadAngle, loadIsGlobal);
    ...
    if (isUniform) {
      ...
      labels.add({
        text: `${casePrefix}${Math.abs(toDisplay(load.qI, 'distributedLoad', dc.unitSystem ?? 'SI')).toFixed(3)} ${unitLabel('distributedLoad', dc.unitSystem ?? 'SI')}${coordLabel}`,
        colour: labelColor,
      ...
      for (const [t, q, end] of [[tStart, load.qI, 'i'], [tEnd, load.qJ, 'j']] as [number, number, string][]) {
        ...
        labels.add({
          text: `${casePrefix}${Math.abs(toDisplay(q, 'distributedLoad', dc.unitSystem ?? 'SI')).toFixed(3)} ${unitLabel('distributedLoad', dc.unitSystem ?? 'SI')}${coordLabel}`,
          colour: labelColor,
...
export function drawPointLoadsOnElements(
  ...
  for (const load of loads) {
    ...
    if (Math.abs(load.p) > 1e-10) {
      ...
      labels.add({
        text: `${ptCasePrefix}${Math.abs(toDisplay(load.p, 'force', dc.unitSystem ?? 'SI')).toFixed(3)} ${unitLabel('force', dc.unitSystem ?? 'SI')}${coordLabel}`,
        colour: ptColor,
        font: '12px sans-serif',
        box: {
          x: fromX + 5, y: fromY - 5,
    ...
    if (Math.abs(px) > 1e-10) {
      ...
      labels.add({
        text: `${ptCasePrefix}${axLabel}=${Math.abs(toDisplay(px, 'force', dc.unitSystem ?? 'SI')).toFixed(3)} ${unitLabel('force', dc.unitSystem ?? 'SI')}`,
        colour: ptColor,
        font: '12px sans-serif',
        box: {
          x: fromX + 5,
    ...
    if (Math.abs(my) > 1e-10) {
      ...
      labels.add({
        text: `${ptCasePrefix}My=${Math.abs(toDisplay(my, 'moment', dc.unitSystem ?? 'SI')).toFixed(3)} ${unitLabel('moment', dc.unitSystem ?? 'SI')}`,
        colour: ptColor,
        font: '12px sans-serif',
        box: {
...
export function drawMovingLoadAxles(
  ...
  for (const axle of axles) {
    ...
    ctx.fillText(`${toDisplay(axle.weight, 'force', dc.unitSystem ?? 'SI').toFixed(2)} ${unitLabel('force', dc.unitSystem ?? 'SI')}`, fromX + snX * 10, fromY + snY * 10);
