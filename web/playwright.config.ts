 ...
 *    the least fragile for the canvas viewport). Firefox/WebKit are a follow-up.
 *
 * Local commands:
...
const E2E_SERVER_COMMAND = process.platform === 'win32'
  ? `set "VITE_E2E=1" && npm run build && npx vite preview --port ${PORT} --host ${HOST} --strictPort`
  : `VITE_E2E=1 npm run build && npx vite preview --port ${PORT} --host ${HOST} --strictPort`;
