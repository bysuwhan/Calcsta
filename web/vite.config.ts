 ...
 * This lets local/static UI checks run without Rust, but a Cloudflare Pages
 * deployment must contain the real solver and therefore fails closed.
 */
function wasmStubPlugin(): Plugin {
  const wasmGlue = resolve(__dirname, 'src/lib/wasm/dedaliano_engine.js');
  const wasmBinary = resolve(__dirname, 'src/lib/wasm/dedaliano_engine_bg.wasm');
  const isCloudflarePages = process.env.CF_PAGES === '1';
  if (isCloudflarePages && (!existsSync(wasmGlue) || !existsSync(wasmBinary))) {
    throw new Error(
      'Dedaliano WASM is missing from a Cloudflare Pages build. '
      + 'Use `npm run build:pages` after installing Rust and wasm-pack.',
    );
  }
  return {
    name: 'wasm-stub',
    ...
    load(id) {
      if (id === '\0wasm-stub') {
        return 'export function initSync() {} export default async function init() {}';
      }
    },
  };
}
