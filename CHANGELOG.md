...
- **Invalidation-based viewport rendering**: the Canvas 2D viewport now renders on demand instead of a continuous 60fps loop. Continuous rendering is reserved for active animations and keyboard navigation.
- **Live calc debounce**: 120ms for Basic 2D prevents solver spam during drag/type operations. Manual solve remains immediate
...
- **Canvas rendering and persistence**: the Basic 2D viewport, `.ded` files, sessions, autosave, and share links now use one planar contract. Legacy spatial payloads fail validation before stores are changed.
- **fixture loading**: separated spring stiffness keys from support opts in `load-fixture.ts`
- **2D load contract**: `fz`/`my` are the canonical nodal load fields (`fy`/`mz` remain serde aliases for backward compatibility); planar bounds use `z_min`/`z_max`, with no spatial fields in product snapshots
...
- **autosave**: `restoreAutosave()` accepts legacy Basic metadata while writing the normalized 2D snapshot

New tests: 39 tests across 4 new test files (model-zcoord, zup-results-sync-projection, fixture-support-metadata, file-save-load). All 1946 web tests and 5919 engine tests pass. (blended count predating the engine-coupled/reference split.)
...
- latest reported full-suite status reached `5590` passing engine-coupled tests with `0` failures
  (plus `1192` reference-formula self-checks counted separately)
